---
title: ゲートウェイテンプレート
description: あなた自身のカスタムWooCommerce POS決済ゲートウェイを作成します
---
...
WooCommerceゲートウェイテンプレートは、WooCommerce POS用の独自のカスタム決済ゲートウェイを作成するための出発点を提供します。このテンプレートには、完全に機能する決済ゲートウェイを構築するために必要なすべてのボイラープレートコードと構造が含まれています。

## 機能 {#features}

<FeatureGrid>
  <FeatureCard icon="code" title="完全なテンプレート">
    必要なすべてのメソッドを持つ準備済みのゲートウェイ構造
  </FeatureCard>
  <FeatureCard icon="store" title="POS統合">
    WooCommerce POS互換性のためにあらかじめ設定されています
  </FeatureCard>
  <FeatureCard icon="terminal" title="自動設定">
    スクリプトベースのテンプレートカスタマイズ
  </FeatureCard>
  <FeatureCard icon="puzzle-piece" title="拡張可能">
    特定の決済プロバイダーのために簡単に変更および拡張可能
  </FeatureCard>
</FeatureGrid>

## 始め方 {#getting-started}

### オプション1: 自動テンプレート生成 {#option-1-automated-template-generation}
...
### オプション2: 手動テンプレートの使用 {#option-2-manual-template-usage}
...
## ゲートウェイのテスト {#testing-your-gateway}
...
### 定期購読サポート {#subscriptions-support}
...
### テンプレートリポジトリ {#template-repository}
