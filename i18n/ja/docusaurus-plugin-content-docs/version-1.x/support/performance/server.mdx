...
WCPOS は、各データ取得操作（商品、注文、顧客など）でサーバーパフォーマンスメトリクスを自動的に収集します。

**v1.10.0** 以降、集計ビューは **ストアヘルス → パフォーマンス** にあります。直近24時間のリクエスト数、転送データ量、代表的な応答時間に加えて、1時間単位の稼働状況ストリップが表示されます（[ストアヘルス](/support/store-health#performance)を参照）。読み取る際の注意点が2つあります。転送データ量の数値は下限値です（サイズを申告しないレスポンスは 0 として計上されます）。また、同期ダイヤルの横に表示される *1日あたり約 N リクエスト* は、設定された間隔から算出した名目上のレートであり、上限でも実測値でもありません。ジッターによって回数は増減し、`304` もリクエストとして計上され、アイドル時の減衰が効くのは60秒より短いプリセットのみです。個々のリクエストのメトリクスは **ストアヘルス → ログ** に表示されます。
...
1. ナビゲーションドロワーから **ストアヘルス** を開き、**パフォーマンス** タブで24時間の状況を確認します（代表的な応答時間が重要な数値です）
2. 時間のかかる操作（商品を同期、注文を作成など）を実行します
3. **ログ** タブで対応するエントリを探します
4. 行を展開してメトリクスを確認します
