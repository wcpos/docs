...
# API03004: 요청이 너무 큽니다 {#api03004-request-too-large}
...
### 1. 요청 크기 줄이기 {#1-reduce-request-size}
...
### 2. 이미지 크기 확인 {#2-check-image-sizes}
...
### 3. 대량 주문 분할 {#3-split-large-orders}

주문에 많은 항목이 있는 경우:
- 여러 주문으로 나누는 것을 고려
- 일반적인 POS 사용에서는 드문 경우

### 4. 서버 제한 늘리기 {#4-increase-server-limits}
...
### 5. 웹 서버 제한 확인 {#5-check-web-server-limits}
