...
# API04006: 리소스 없음 {#api04006-resource-not-found}
...
- **고유주소 문제** — WordPress 고유주소를 새로고침해야 합니다
...
### 1. 리소스 존재 여부 확인 {#1-check-if-the-resource-exists}

리소스가 여전히 귀하의 WooCommerce 사이트에 존재하는지 확인합니다:
- WordPress 관리자에 로그인하세요
- 관련 섹션(제품, 주문, 고객)으로 이동
- ID 또는 이름으로 항목 검색

### 2. 로컬 데이터 새로 고침 {#2-refresh-local-data}
...
### 3. WordPress 고유주소 확인 {#3-check-wordpress-permalinks}

여러 리소스가 발견되지 않는 경우:
1. WordPress 관리자에서 **설정 → 고유주소**로 이동하세요
2. **변경 사항 저장**을 클릭합니다 (변경 없이도)
3. 이로써 퍼머링크 구조가 새로 고쳐집니다

### 4. API 경로 확인 {#4-verify-api-routes}
