...
# API04005: JSON 복구 시도 {#api04005-json-recovery-attempted}

## 이것의 의미 {#what-this-means}

서버가 일부 잘못된 JSON 내용을 포함한 응답을 보냈지만, POS는 이를 복구하고 유효한 데이터를 추출하려고 시도했습니다. 이것은 중요한 오류가 아닌 정보성 알림입니다.

## 무엇이 발생했나요? {#what-happened}
...
### 1. PHP 오류 표시 비활성화 {#1-disable-php-display-errors}
...
### 2. 대신 오류 로깅 활성화 {#2-enable-error-logging-instead}
...
### 3. 플러그인 디버그 모드 확인 {#3-check-for-plugin-debug-mode}
...
### 4. 최근 변경 사항 검토 {#4-review-recent-changes}
