...
# API03006: 지원하지 않는 메서드 {#api03006-unsupported-method}
...
### 1. 두 구성 요소 업데이트 {#1-update-both-components}

호환성을 보장합니다:
- WCPOS WordPress 플러그인을 업데이트하세요
- POS 애플리케이션 업데이트
- 파손된 변경 사항에 대한 릴리스 노트 확인

### 2. 서버 구성 확인 {#2-check-server-configuration}
...
### 3. REST API 접근 확인 {#3-verify-rest-api-access}
...
### 4. 플러그인 충돌 확인 {#4-check-for-plugin-conflicts}
...
### 5. 호스팅 제한 검토 {#5-review-hosting-restrictions}
