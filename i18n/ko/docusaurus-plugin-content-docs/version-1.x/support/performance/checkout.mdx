...
## WCPOS 결제가 작동하는 방식 {#how-wcpos-checkout-works}

WCPOS 결제는 WooCommerce 주문 결제 템플릿의 축소된 버전이 로드된 iframe/webview가 있는 모달을 엽니다. 이는 고객이 온라인으로 주문 결제를 할 때 보는 [고객 결제 페이지](https://woocommerce.com/document/managing-orders/paying-for-orders/#customer-payment-page-link)와 본질적으로 동일한 페이지입니다.
...
### 1. 불필요한 스크립트와 스타일 {#1-unnecessary-scripts-and-styles}

**문제:** 테마와 플러그인은 결제에 필요하지 않은 스크립트와 스타일을 추가하여 페이지 로드 시간을 느리게 만듭니다.

**해결책:** [결제 설정](/pos/checkout#checkout-settings-troubleshooting)을 사용하여 불필요한 리소스를 비활성화하세요:

1. 결제 모달에서 **결제 설정** 버튼을 클릭하세요.
...
### 2. 주문 생성 중 플러그인 간섭 {#2-plugin-interference-during-order-creation}
...
### 3. 서버 자원 한계 {#3-server-resource-limitations}
...
## 워드프레스 개발자 모범 사례 {#wordpress-developer-best-practices}

:::tip 개발자를 위한 팁
워드프레스 개발자는 `wp_enqueue_scripts`를 사용하여 페이지에 스크립트와 스타일을 적절하게 추가해야 합니다. 불행히도 일부 플러그인 및 테마 개발자는 모범 사례를 따르지 않고 코드를 템플릿에 직접 삽입하여 이러한 리소스를 쉽게 제거할 수 없게 만듭니다.
:::

결제 설정을 통해 비활성화할 수 없는 스크립트나 스타일에 문제가 있는 경우:

1. **개발자에게 문의** - 그들에게 워드프레스 코딩 표준을 따르도록 요청하세요.
2. **자식 테마 사용** - 자식 테마에서 문제의 템플릿을 오버라이드하세요.
3. **사용자 정의 훅** - 결제 페이지에서 문제의 코드를 제거하기 위해 워드프레스 훅을 사용하세요.
...
- 결제 시간이 consistently 10초 이상 소요되는 경우
