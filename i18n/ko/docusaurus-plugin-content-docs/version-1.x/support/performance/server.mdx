...
WCPOS는 각 데이터 가져오기 작업(제품, 주문, 고객 등)마다 서버 성능 메트릭을 자동으로 수집합니다.

**v1.10.0**부터 종합 보기는 **스토어 상태 → 성능**에 있습니다. 지난 24시간 동안의 요청 수, 전송된 데이터량, 일반적인 응답 시간과 시간별 가동 상태 스트립을 보여줍니다([스토어 상태](/support/store-health#performance) 참조). 읽을 때 두 가지 주의할 점이 있습니다. 전송된 데이터량 수치는 하한값이며(크기를 명시하지 않는 응답은 0으로 계산됩니다), 동기화 다이얼 옆의 *하루 약 N건 요청* 추정치는 설정된 간격에서 계산한 명목 비율일 뿐 상한이나 실제 횟수가 아닙니다. 지터에 따라 횟수는 양방향으로 변하고, `304` 응답도 요청 1건으로 계산되며, 유휴 감쇠는 60초보다 빠른 프리셋에서만 횟수를 낮춥니다. 개별 요청 메트릭은 **스토어 상태 → 로그**에 표시됩니다.
...
1. 내비게이션 창에서 **스토어 상태**를 열고 **성능** 탭에서 24시간 현황을 확인합니다(일반적인 응답 시간이 핵심 수치입니다).
2. 느린 작업을 수행합니다(제품 동기화, 주문 생성 등).
3. **로그** 탭에서 해당 항목을 찾습니다.
4. 행을 확장하여 메트릭을 확인합니다.
