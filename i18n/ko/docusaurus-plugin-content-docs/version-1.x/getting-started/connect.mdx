...
<Accordion>
  ...
  - **보안 플러그인** — Wordfence, Really Simple Security, WPS Hide Login, iThemes/Solid Security, Defender Pro가 흔한 원인입니다. 전체 목록과 해결 방법은 [플러그인 충돌](/support/troubleshooting/plugin-conflicts#security-and-login-plugins)에서 확인하세요.
  ...
  **해결 방법:** 보안 플러그인이 `/wp-json/wcpos/` 및 `/wp-json/wc/v3/` 에 대한 비인증 접근을 허용하도록 구성하거나, 첫 연결을 완료하는 동안만 비활성화하세요.
  ...
  - 호스트가 REST API를 차단하고 있지 않은지 확인하세요. [호스팅별 참고 사항](/support/performance/server#hosting-specific-notes)을 참조하세요
  - 서버가 [최소 요구 사항](/support/performance/server#minimum-server-requirements)을 충족하는지 확인하세요
