...
<Accordion>
  ...
  **해결 방법:** 보안 플러그인이 `/wp-json/wcpos/` 및 `/wp-json/wc/v3/` 에 대한 비인증 접근을 허용하도록 구성하거나, 첫 연결을 완료하는 동안만 비활성화하세요.
  ...
  앱과 서버 플러그인의 버전이 맞지 않는지 확인하세요. WCPOS 플러그인은 아직 업데이트가 필요한데 앱 스토어에서 앱 업데이트가 배포되었을 수 있습니다(또는 그 반대일 수 있습니다). v1.10에서는 앱이 플러그인의 v2 동기화 API를 사용하므로 앱과 WCPOS 플러그인이 모두 **v1.10 릴리스 라인**에 있어야 합니다. 한쪽만 업데이트되어 요청이 `rest_no_route`로 실패하는 경우 나머지 한쪽도 업데이트하세요.
