...
| **Simple Thermal Receipt (80mm)** | Térmica | Layout térmico limpo de 80mm — o mais comum |
| **Simple Thermal Receipt 80mm (RTL)** | Térmica | Versão RTL para 80mm. Requer uma impressora com codepage árabe |
| **Detailed Thermal Receipt (58mm / 80mm)** | Térmica | Inclui detalhamento de impostos, endereços, reembolsos, pagamentos, termos e código de barras |
| **Kitchen Ticket** | Térmica | Apenas itens, fonte grande, sem preços — para estações de preparo |
...
:::info E as variáveis?
Os trechos como `{{store.name}}` e `{{order.number}}` são **espaços reservados** para seus dados reais. A IA entende esses marcadores — você não precisa se preocupar com eles. Se quiser conhecer todos os espaços reservados disponíveis, consulte a [Referência de Dados do Recibo](/templates/receipt-data).
:::
...
- **[Modelos HTML](/templates/html-templates)** — Marcadores no estilo Mustache com `{{variable}}`. Renderiza no lado do cliente, funciona offline. **Recomendado para a maioria das lojas.**
- **[Modelos Térmicos](/templates/thermal-templates)** — XML para impressoras térmicas ESC/POS. O mesmo modelo produz tanto a pré-visualização na tela quanto a saída da impressora.
- **[Referência de Dados do Recibo](/templates/receipt-data)** — Todos os marcadores que você pode usar, agrupados por seção.
