...
<FeatureGrid>
  <FeatureCard icon="code" title="Modelo Completo">
    Estrutura de gateway pronta para uso com todos os métodos exigidos
  </FeatureCard>
  <FeatureCard icon="store" title="Integração POS">
    Pré-configurado para compatibilidade com WooCommerce POS
  </FeatureCard>
  <FeatureCard icon="terminal" title="Configuração Automatizada">
    Customização do modelo baseada em script
  </FeatureCard>
...
### Opção 1: Geração Automatizada de Modelo {#option-1-automated-template-generation}
...
### Opção 2: Uso Manual do Modelo {#option-2-manual-template-usage}

Se você preferir uma customização manual:
...
### Arquivo Principal do Plugin {#main-plugin-file}
...
## Guia de Customização {#customisation-guide}
...
### Processamento de Pagamento {#payment-processing}
...
### Configurações do Administrador {#admin-settings}
...
### Integração POS {#pos-integration}
...
## Testando Seu Gateway {#testing-your-gateway}
...
### Empacotamento do Plugin {#plugin-packaging}
...
### Suporte a Assinaturas {#subscriptions-support}
...
### Repositório do Template {#template-repository}
