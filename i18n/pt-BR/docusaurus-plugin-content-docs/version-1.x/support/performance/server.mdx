...
O WCPOS coleta automaticamente métricas de desempenho do servidor em cada operação de busca de dados (produtos, pedidos, clientes etc.).

A partir da **v1.10.0**, a visão agregada fica em **Saúde da loja → Desempenho**: requisições feitas, dados transferidos e tempo de resposta típico das últimas 24 horas, além de uma faixa de disponibilidade por hora (veja [Saúde da loja](/support/store-health#performance)). Duas ressalvas ao interpretar esses números: o valor de dados transferidos é um piso (respostas que não declaram um tamanho contribuem com zero), e a estimativa de *~N requisições por dia* ao lado do seletor de sincronização é uma taxa nominal derivada do intervalo configurado, não um teto nem uma contagem real. O jitter move a contagem nos dois sentidos, um `304` ainda conta como requisição, e o decaimento por ociosidade só a reduz em predefinições mais rápidas que 60 segundos. As métricas de requisições individuais aparecem em **Saúde da loja → Logs**.
...
1. Abra **Saúde da loja** na gaveta de navegação e verifique a aba **Desempenho** para o panorama de 24 horas (o tempo de resposta típico é o número principal)
2. Execute a operação lenta (sincronizar produtos, criar pedido etc.)
3. Procure a entrada correspondente na aba **Logs**
4. Expanda a linha para ver as métricas
