...
sidebar_position: 2
description: Otimize o desempenho do servidor WordPress para WCPOS, incluindo HPOS, cache, otimização de banco de dados e recomendações de hospedagem.
---
...
## Compreendendo as Métricas do Servidor {#understanding-server-metrics}

O WCPOS coleta automaticamente métricas de desempenho do servidor a cada operação de busca de dados (produtos, pedidos, clientes, etc.). Você pode visualizar essas métricas na tela de [Logs](/support/logs).
...
### Passo 1: Monitore os Logs {#step-1-monitor-the-logs}

1. Abra **Logs** no painel de navegação
2. Realize a operação lenta (sinc de produtos, criação de pedido, etc.)
3. Procure a entrada de log correspondente
4. Expanda o contexto para ver as métricas

### Passo 2: Analise as Métricas {#step-2-analyse-the-metrics}
...
### 1. Recursos do Servidor Insuficientes {#1-insufficient-server-resources}
...
- **Usar cache de objetos** - Redis ou Memcached, se disponíveis no seu servidor
...
## Melhores Práticas de Monitoramento do Servidor {#server-monitoring-best-practices}

### 1. Verificações de Desempenho Regulares {#1-regular-performance-checks}
...
### 2. Defina Alertas de Desempenho {#2-set-performance-alerts}
...
### 1. Melhores Práticas do WordPress/WooCommerce {#1-wordpresswoocommerce-best-practices}
...
### 2. Otimizações a Nível de Hospedagem {#2-hosting-level-optimizations}
