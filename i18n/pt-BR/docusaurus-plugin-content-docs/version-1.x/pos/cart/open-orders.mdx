...
sidebar_position: 3
description: "Trabalhe com múltiplos pedidos simultaneamente no WCPOS, incluindo pedidos em espera, alternando entre transações e recuperando-se de interrupções."
---

O WCPOS permite que você trabalhe com múltiplos pedidos simultaneamente. Isso é útil para lidar com clientes que se afastam, alternar entre transações e recuperar-se de interrupções.

## Carrossel de Pedidos Abertos {#open-orders-carousel}
...
## Criando um Novo Pedido {#creating-a-new-order}
...
## Alternando Entre Pedidos {#switching-between-orders}
...
## Salvando Pedidos no Servidor {#saving-orders-to-server}
...
### Salvos no Servidor {#saved-to-server}
...
- Você estiver encerrando seu turno e outro caixa continuará
- Você quiser um backup em caso de problemas com o app/navegador
:::

## Recuperando Pedidos Salvos {#recovering-saved-orders}
...
## Persistência de Pedidos {#order-persistence}
...
### Sincronização com o Servidor {#sync-with-server}
...
## Cancelando Pedidos {#voiding-orders}
...
Para recuperar um pedido salvo que foi cancelado:
1. Vá para `WP Admin > WooCommerce > Pedidos > Lixeira`
2. Restaure o pedido

## Dicas {#tips}

### Mantenha os Pedidos Organizados {#keep-orders-organized}
...
### Transferências de Turno {#shift-handoffs}
...
### Considerações Offline {#offline-considerations}

Se você perder conectividade:

- Pedidos locais permanecem acessíveis e você pode continuar adicionando itens
- Você não pode concluir/finalizar pedidos até que a conectividade seja restaurada
