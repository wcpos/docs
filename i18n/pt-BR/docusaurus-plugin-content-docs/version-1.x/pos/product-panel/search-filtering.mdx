...
- **Correspondência de Substring** - Encontra seu termo em qualquer posição dentro de uma palavra (por exemplo, pesquisar "berry" encontra "blueberry")
...
:::note Os termos de busca precisam de pelo menos 3 caracteres
A partir da versão 1.10.0, a busca corresponde a substrings **dentro** das palavras — "berry" encontra "blueberry", e "saippua" encontra "Kuorintasaippua". O contrapeso é um comprimento mínimo de termo: consultas com menos de **3 caracteres** não são comparadas com o índice de texto. Códigos curtos continuam funcionando por meio das buscas por código de barras e SKU.

Antes da versão 1.10.0, a correspondência era apenas por prefixo (a busca encontrava o início de uma palavra, nunca o meio).
:::
...
3. **Consulta o servidor** em busca dos produtos correspondentes caso ainda não estejam locais, e os armazena para que a mesma busca seja instantânea da próxima vez
4. **Atualiza os resultados** em tempo real conforme você digita

A busca traz aquilo que você pesquisou — o restante do seu catálogo é baixado pela carga inicial em segundo plano (veja [Sincronização de Produtos](/products/sync)), de modo que a busca continua rápida sem trazer dados não relacionados.
...
A carga inicial do catálogo em segundo plano pode ainda não ter terminado. Se um produto não aparecer:

1. **Pesquise por ele** - Isso busca os produtos correspondentes diretamente no seu servidor caso ainda não estejam locais
2. **Verifique o progresso do download** - **Saúde da loja → Banco de dados** mostra quanto do seu catálogo já está neste dispositivo
3. **Verifique a visibilidade dele** - Produtos sem estoque ficam ocultos por padrão, e apenas os tipos de produto padrão do WooCommerce são compatíveis

Saiba mais no nosso guia de [Sincronização de Produtos](/products/sync).

  </AccordionItem>

  <AccordionItem question="É possível pesquisar por nomes parciais de produtos ou SKUs?">

Sim! A busca tokenizada corresponde a substrings, o que significa que:

- Pesquisar "blue" encontrará produtos com "blueberry", "blue shirt", etc.
- Pesquisar "berry" também encontrará "blueberry" — o termo pode aparecer em qualquer posição da palavra
- Pesquisar "ABC" encontrará SKUs como "ABC123", "ABC-XYZ", etc.
- Não é necessário digitar palavras ou códigos completos (mas termos com menos de 3 caracteres não são correspondidos)
