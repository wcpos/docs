...
WCPOS raccoglie automaticamente le metriche delle prestazioni del server a ogni operazione di recupero dei dati (prodotti, ordini, clienti, ecc.).

Dalla versione **1.10.0**, la vista aggregata si trova in **Stato del negozio → Prestazioni**: richieste effettuate, dati trasferiti e tempo di risposta tipico nelle ultime 24 ore, oltre a una striscia oraria di disponibilità (vedi [Stato del negozio](/support/store-health#performance)). Due avvertenze nel leggerla: il valore dei dati trasferiti è un limite inferiore (le risposte che non dichiarano una dimensione contribuiscono per zero) e la stima *~N richieste al giorno* accanto al regolatore di sincronizzazione è un tasso nominale ricavato dall'intervallo configurato, non un tetto massimo né un conteggio effettivo. Il jitter sposta il conteggio in entrambe le direzioni, un `304` conta comunque come una richiesta e il decadimento in inattività lo abbassa solo per i preset più rapidi di 60 secondi. Le metriche delle singole richieste compaiono in **Stato del negozio → Log**.
...
1. Aprire **Stato del negozio** dal menu di navigazione e consultare la scheda **Prestazioni** per il quadro delle 24 ore (il tempo di risposta tipico è il valore chiave)
2. Eseguire l'operazione lenta (sincronizzazione dei prodotti, creazione di un ordine, ecc.)
3. Cercare la voce corrispondente nella scheda **Log**
4. Espandere la riga per visualizzare le metriche
