...
description: "Configura i ruoli e le capacità degli utenti per l'accesso a WCPOS, inclusi i permessi di Amministratore, Shop Manager e Cassiere."
---
...
## Cosa può fare ciascun ruolo predefinito, in breve {#what-each-default-role-can-do-at-a-glance}
...
Per la suddivisione completa per singolo endpoint — inclusi gli endpoint del plugin Pro, il comportamento di scadenza dei token e i suggerimenti per distinguere gli errori di capacità dagli errori di token — vedi il [riferimento Accesso agli endpoint per ruolo](/reference/role-endpoint-access).
