...
1. **Plugin PHP:** Ospitato sul tuo server, è un plugin relativamente piccolo che estende l'[API REST di WooCommerce](/reference/wc-rest-api) con endpoint specifici per il POS.
...
- Il _mondo PHP_ è dove avviene la gestione dei dati usando WordPress e WooCommerce.
...
:::info Modificato nella v1.10.0
La v1.10.0 sostituisce il precedente livello di replica con un **motore di sincronizzazione** dedicato. Il riepilogo qui sotto è la versione breve — la spiegazione completa si trova in [Come funziona il motore di sincronizzazione](/reference/sync-engine).
:::
...
| Buono 😊 | Cattivo 😟 |
| ------------------------------------------------------------------ | -------------------------------------------------------- |
| La ricerca dei dati locali è istantanea | Mantenere i dati sincronizzati è una sfida |
...
## Estensioni dell'API {#api-extensions}

Il plugin PHP estende l'API REST di WooCommerce con endpoint aggiuntivi per funzionalità specifiche del POS, registrati sotto i namespace dedicati `wcpos/v1` e `wcpos/v2` — `wcpos/v2` contiene la superficie di sincronizzazione della v1.10.0, ed è per questo che le versioni dell'app e del plugin vengono rilasciate di pari passo. Vedi [API REST di WooCommerce](/reference/wc-rest-api) per un'introduzione.

### Il namespace `wcpos/v2` {#wcpos-v2-namespace}
...
- **Endpoint pubblici.** `wcpos/v2/site`, `wcpos/v2/ping` e `wcpos/v2/echo` sono pubblici (usati per il rilevamento delle capacità e della connettività, incluse le [riserve di trasporto](/error-codes) per gli host restrittivi).
...
- **La memorizzazione dei prezzi per singola riga è documentata** per la compatibilità di sincronizzazione con terze parti — vedi [Come vengono memorizzate le modifiche di prezzo del POS](/reference/pos-discounts#how-pos-price-overrides-are-stored).
