...
Onderdelen zoals `{{store.name}}` en `{{order.number}}` zijn **placeholders** voor uw echte gegevens. De AI begrijpt deze; u hoeft dat niet te doen. Als u alle beschikbare placeholders wilt kennen, bekijk dan de [Referentie voor bongegevens](/templates/receipt-data).
:::
...
- **[HTML-templates](/templates/html-templates)** — Mustache-achtige placeholders `{{variable}}`. Worden client-side gerenderd en werken offline. **Aanbevolen voor de meeste winkels.**
- **[Thermische templates](/templates/thermal-templates)** — XML voor ESC/POS-thermische printers. Dezelfde template levert zowel het voorbeeld op het scherm als de printeruitvoer.
- **[Referentie voor bongegevens](/templates/receipt-data)** — Elke placeholder die u kunt gebruiken, gegroepeerd per sectie.
...
```html
<div style="font-style: italic; color: #6b7280;">Familiebedrijf sinds 1987</div>
```

In een thermische template:

```xml
<align mode="center"><text>Familiebedrijf sinds 1987</text></align>
```
...
```html
<!-- Standaard: Code 128-barcode van het bestelnummer -->
<barcode type="code128">{{order.number}}</barcode>

<!-- Overschakelen naar een QR-code -->
<barcode type="qrcode">{{order.number}}</barcode>

<!-- Of codeer iets anders, zoals een retour-URL -->
