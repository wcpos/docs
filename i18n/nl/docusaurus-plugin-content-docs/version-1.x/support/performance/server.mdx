...
WCPOS verzamelt automatisch serverprestatiestatistieken bij elke gegevensophaalbewerking (producten, bestellingen, klanten, enz.).

Vanaf **v1.10.0** staat het totaaloverzicht in **Winkelstatus → Prestaties**: uitgevoerde verzoeken, overgedragen gegevens en de typische reactietijd van de afgelopen 24 uur, plus een uptimebalk per uur (zie [Winkelstatus](/support/store-health#performance)). Twee kanttekeningen bij het lezen ervan: het cijfer voor overgedragen gegevens is een ondergrens (reacties die geen omvang opgeven, tellen als nul) en de schatting *~N verzoeken per dag* naast de synchronisatieschijf is een nominaal tempo dat is afgeleid van het ingestelde interval, geen bovengrens en geen werkelijk aantal. Jitter verschuift het aantal in beide richtingen, een `304` telt nog steeds als een verzoek en inactiviteitsafbouw verlaagt het alleen bij voorinstellingen die sneller zijn dan 60 seconden. Statistieken van afzonderlijke verzoeken staan in **Winkelstatus → Logs**.
...
1. Open **Winkelstatus** vanuit de navigatielade en bekijk het tabblad **Prestaties** voor het beeld over 24 uur (de typische reactietijd is het belangrijkste getal)
2. Voer de langzame handeling uit (producten synchroniseren, bestelling plaatsen, etc.)
3. Zoek naar de bijbehorende vermelding op het tabblad **Logs**
4. Vouw de rij uit om statistieken te bekijken
