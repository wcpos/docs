...
sidebar_label: 结账
sidebar_position: 1
description: 在 WCPOS 结账中完成销售，包括支付处理、分摊付款和订单完成。
---
...
当您准备完成销售时，单击 **结账** 按钮以打开结账模式。在这里，您可以处理付款并完成订单。

{/* 截图: checkout-modal.png */}

## 结账模式概述 {#checkout-modal-overview}
...
### 额外网关 (Pro) {#additional-gateways-pro}
...
2. 对于现金，输入投入金额
3. 单击 **处理付款**
4. 订单完成，显示 [收据](/receipts/at-checkout)

## 结账设置（故障排除） {#checkout-settings-troubleshooting}

结账模式使用 iframe/webview 以显示 WooCommerce 订单支付页面。这利用了 WooCommerce 现有的支付基础设施，这意味着任何与 WooCommerce 兼容的支付网关都应该可以在 POS 中使用。
...
<div class="image-container">
  <Image
    ...
    style={{ maxHeight: 600 }}
  />
  <p class="image-caption">结账模式中的结账设置</p>
</div>
...
:::warning 核选项
这是核选项，仅应在测试或开发者知道自己在做什么的罕见情况下使用。禁用所有 wp_head 脚本将删除即使是 WooCommerce 所需的脚本，可能会破坏支付功能。
:::
...
单击 **取消** 以关闭结账模式而不完成订单。订单将保持为开放购物车。

## 相关文档 {#related-documentation}

<LinkCards>
  <LinkCard to="/receipts/at-checkout" title="收据" description="结账后，打印或发送收据" />
