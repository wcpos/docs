...
# API02006: API 密钥无效 {#api02006-api-key-invalid}
...
### 1. 验证 API 密钥 {#1-verify-the-api-key}
...
### 2. 生成新的 API 密钥 {#2-generate-new-api-keys}
...
### 3. 更新 POS 配置 {#3-update-pos-configuration}

在 POS 中输入新的 API 密钥：
- Consumer Key（以 `ck_` 开头）
- Consumer Secret（以 `cs_` 开头）

### 4. 检查密钥权限 {#4-check-key-permissions}
