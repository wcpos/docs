...
# API03004: 请求过大 {#api03004-request-too-large}
...
### 1. 减少请求大小 {#1-reduce-request-size}
...
### 3. 拆分大订单 {#3-split-large-orders}

如果一个订单包含许多项目：
- 考虑拆分为多个订单
- 这种情况在正常POS使用中很少见

### 4. 增加服务器限制 {#4-increase-server-limits}
...
### 5. 检查Web服务器限制 {#5-check-web-server-limits}
