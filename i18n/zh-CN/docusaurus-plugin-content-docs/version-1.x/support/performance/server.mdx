...
WCPOS 在每次数据获取操作（产品、订单、客户等）时自动收集服务器性能指标。

从 **v1.10.0** 起，汇总视图位于 **商店健康 → 性能**：过去 24 小时内发出的请求数、传输的数据量和典型响应时间，以及按小时显示的正常运行状态条（参见[商店健康](/support/store-health#performance)）。解读时需注意两点：数据传输量是一个下限值（未声明大小的响应按零计算）；同步刻度盘旁的*每天约 N 次请求*是根据配置的间隔推算出的名义速率，既不是上限也不是实际计数。抖动会使该计数上下浮动，`304` 响应同样计为一次请求，而空闲衰减仅会降低间隔短于 60 秒的预设的请求数。单次请求的指标可在 **商店健康 → 日志** 中查看。
...
1. 从导航抽屉中打开 **商店健康**，并查看 **性能** 选项卡以了解 24 小时内的整体情况（典型响应时间是关键数值）
2. 执行慢操作（同步产品、创建订单等）
3. 在 **日志** 选项卡中查找相应的条目
4. 展开该行以查看指标
