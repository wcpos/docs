...
:::info 关于变量
类似 `{{store.name}}` 和 `{{order.number}}` 的部分是真实数据的**占位符**。AI 能够理解这些占位符——您无需了解细节。如果您想知道所有可用的占位符，请参阅[收据数据参考](/receipts/receipt-data)。
:::
...
- **[HTML 模板](/receipts/html-templates)** — Mustache 风格的 `{{variable}}` 占位符。客户端渲染，支持离线使用。**推荐大多数店铺使用。**
- **[热敏打印模板](/receipts/thermal-templates)** — 用于 ESC/POS 热敏打印机的 XML。同一模板同时生成屏幕预览和打印输出。
- **[小票数据参考](/receipts/receipt-data)** — 所有可用占位符，按分区分组。
