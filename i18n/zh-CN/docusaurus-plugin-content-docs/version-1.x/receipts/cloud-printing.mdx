...
## WCPOS Cloud Print——中继 {#relay}

当您设置**轮询**打印机（Star CloudPRNT 或 Epson Server Direct Print）时，WCPOS 提供给您的轮询 URL 指向的是 **cloudprint.wcpos.com**，而不是您自己的站点。那就是 **WCPOS Cloud Print**——由 WCPOS 运营的免费中继，位于您的打印机与商店之间。

**它为何存在。** 收据打印机运行的是体积小、很少更新的固件。许多打印机无法与运行在现代主机上的商店完成安全连接：较新的 TLS 版本、较新的证书颁发机构，或站点前方的 CDN/防火墙（Cloudflare 是常见原因）会拒绝打印机——或者被打印机拒绝。表现出来的症状就是即使每项设置都正确，打印机也永远停留在 **Waiting**。中继向打印机提供一种旧固件始终能够协商成功的连接，并将它们的流量转发到您的商店。

**它的工作方式：**

- 当您首次打开 Cloud Print 设置时，您的站点会**自动**连接到中继——无需启用或配置任何内容。
- 打印机轮询 `cloudprint.wcpos.com`；中继将请求转发到您的商店，并返回您商店的响应。打印任务只是**经过**中继——绝不会存储在其上。
- 中继吸收了大部分空闲轮询，因此一台空闲打印机对您站点的开销约为每分钟一次请求，而不是每几秒一次。
- 如果您的站点无法连接到中继（例如中继无法访问的本地开发站点），WCPOS 会自动回退到直连站点的 URL。

:::note 更倾向于直接连接？
每台打印机的设置界面都有一个 **Advanced** 展开项，其中显示直连站点的 URL。如果您确定自己的主机对打印机友好，或者不希望让任务经过中继，可以使用它。开发者可以用一行代码在全站关闭中继：
...
**中继能看到什么。** 连接会将您的站点 URL 共享给 wcpos.com，并且打印任务——包含订单详情的收据内容——会在送往打印机的途中经过中继。不会保留任何内容。完整详情见[隐私政策](https://wcpos.com/privacy)。
...
复制生成的轮询 URL 和令牌。该 URL 通常指向 **cloudprint.wcpos.com**——那就是 [WCPOS Cloud Print 中继](#relay)，也正是您所需要的；直连站点的 URL 位于 **Advanced** 展开项之下。该**令牌只显示一次**——如果丢失，请从打印机卡片重新生成一个新令牌，并用新值更新打印机。
...
发往云打印机的每个任务都会在您的站点上排队，直到打印机将其取走（轮询打印机）或提供商确认投递为止。**WP Admin > POS > Settings > Cloud Print** 会在打印机下方显示该队列：

- 按打印机或状态**筛选**——**Waiting**、**Printing**、**Printed**、**Failed**、**Cancelled**——并附有实时计数。
- 每一行都会显示该收据已**等待**多长时间，并链接到它所打印的 WooCommerce **订单**。
- 可以逐个、批量或针对整台打印机一次性**取消**等待中的任务。**重试**会将失败的任务重新排队。

如果某台打印机有等待中的任务，但超过几分钟没有取走任何内容，则会出现一条**警告横幅**，指明该打印机、积压了多少张收据以及积压了多久，并提供**全部取消**——打印机被拔掉时，积压绝不会悄无声息地累积。
...
<Accordion>
  ...
  - 如果您输入的是**直连站点的 URL**（来自 Advanced 展开项）而打印机仍无法连接，请改用标准的 **cloudprint.wcpos.com** URL——许多打印机无法与现代主机协商安全连接，而[这正是中继要解决的问题](#relay)。

  </AccordionItem>

  <AccordionItem question="为什么我的打印机 URL 指向 cloudprint.wcpos.com？">

  那就是 [WCPOS Cloud Print](#relay)，这个免费中继为打印机提供一种其固件始终能够协商成功的连接，并将它们的打印任务转发到您的商店。它会自动启用，任务绝不会存储在其上；如果您更倾向于直连，打印机的 **Advanced** 展开项下始终提供直连站点的 URL。

  </AccordionItem>

  <AccordionItem question="我在中继出现之前就设置好了打印机——需要更改什么吗？">

  不需要——已保存在打印机配置中的 URL 会继续有效。不过，如果该打印机**卡在 Waiting**，那么它的固件很可能无法直接连接到您的主机：请在打印机卡片上打开 **Setup &amp; token**，重新生成令牌，然后在打印机配置中输入新的 **cloudprint.wcpos.com** URL 和令牌。
