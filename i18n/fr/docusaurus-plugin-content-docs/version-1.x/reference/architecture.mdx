...
1. **Plugin PHP :** Hébergé sur votre serveur, c'est un plugin relativement petit qui étend l'[API REST de WooCommerce](/reference/wc-rest-api).
...
### Qu'est-ce qui est Synchronisé ? {#what-gets-synced}
...
### Stratégie de Synchronisation {#sync-strategy}
...
Le plugin PHP étend l'API REST de WooCommerce avec des points de terminaison supplémentaires pour des fonctionnalités spécifiques au POS. Consultez l'[API REST de WooCommerce](/reference/wc-rest-api) pour plus de détails.
