...
- Si vous utilisez un terminal avec règlement de lot manuel, **réglez le lot maintenant**, avant de partir. Les passerelles intégrées comme Stripe Terminal règlent automatiquement. Pour la configuration Square via le paiement en ligne, consultez [Paiement en ligne](/payment/custom-gateways/web-checkout).

</StepCard>
  <StepCard number="6" title="Imprimer le rapport">

Dans le panneau Résumé de l'écran Rapports, cliquez sur **Imprimer**. Cette action utilise généralement le routage d'impression configuré (voir [Routage d'impression](/templates/printer-setup#print-routing)) pour envoyer le rapport vers une imprimante de reçus ou de documents et produit un enregistrement papier.
