...
# API05001 : API WooCommerce désactivée {#api05001-woocommerce-api-disabled}
...
### 1. Activer l'API REST WooCommerce {#1-enable-woocommerce-rest-api}
...
### 2. Vérifier l'API REST WordPress {#2-check-wordpress-rest-api}

L'API WooCommerce dépend de l'API REST WordPress :
1. Ouvrez `https://yoursite.com/wp-json/` dans votre navigateur
2. Cela devrait renvoyer des données JSON
3. Si ce n'est pas le cas, consultez [API05004](./API05004)

### 3. Vérifier les plugins de sécurité {#3-check-security-plugins}
...
### 5. Actualiser les permaliens {#5-flush-permalinks}
