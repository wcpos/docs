...
1. Ne relancez pas encore la validation de commande, et ne débitez pas à nouveau le client.
2. Ouvrez WooCommerce → Commandes dans WP Admin et cherchez la commande — recherchez par le total et sur les dernières minutes.
3. Si la commande existe, la vente a bien abouti — ne la recréez pas ; synchronisez le POS pour qu’elle apparaisse sur la caisse.
4. Si aucune commande n’existe après une synchronisation, relancez la validation de commande une fois.
5. Toujours incertain ? Exportez les infos de débogage et contactez l’assistance avant toute nouvelle tentative.
