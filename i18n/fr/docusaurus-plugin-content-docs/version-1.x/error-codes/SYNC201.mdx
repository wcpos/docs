---
title: "SYNC201 : Enregistrement refusé"
sidebar_label: SYNC201
description: "Cet enregistrement a été refusé par votre boutique et n’est enregistré que sur cet appareil."
---

{/* GENERATED PAGE — do not edit. Source of truth: packages/utils/src/logger/error-registry.json in wcpos/monorepo. Regenerate with `pnpm generate:error-codes`. */}

## Ce que cela signifie {#what-this-means}

Cet enregistrement a été refusé par votre boutique et n’est enregistré que sur cet appareil.

Corrigez la cause indiquée pour l’enregistrement identifié, puis réessayez. Exportez les diagnostics s’il reste refusé.

## Que faire {#what-to-do}

Corrigez les informations signalées, puis réessayez. Réessayez après avoir effectué la modification ci-dessus ; réessayer sans elle échouera de la même manière.

## Vos données {#your-data}

La modification est enregistrée sur cet appareil mais n’est pas parvenue à votre boutique. Si le problème persiste, exportez les diagnostics et contactez l’assistance WCPOS.

## Dépannage {#troubleshoot}

1. Dépliez l’entrée de journal : elle indique l’enregistrement refusé et contient la raison donnée par la boutique (serverCode) pour ce refus.
2. Les enregistrements refusés sont listés dans État de la boutique — ouvrez l’enregistrement, corrigez l’élément refusé par la boutique, et relancez-le depuis là.
3. Si la raison mentionne un champ que le POS n’affiche pas, ouvrez le même enregistrement dans WP Admin et vérifiez-le là-bas — une autre extension a peut-être ajouté une validation invisible pour le POS.
4. S’il reste refusé après modification, utilisez Copier les infos de débogage et contactez l’assistance en indiquant le serverCode.

## Où chercher {#where-to-look}

Lorsque WCPOS peut enregistrer cette erreur, elle est consignée sur l’appareil qui l’a déclenchée. Ouvrez **État de la boutique → Journaux** (l’icône en forme de cœur avec pouls, en bas du tiroir de navigation), repérez l’entrée portant ce code et dépliez-la : la ligne dépliée indique la raison en langage clair ainsi que le contexte capturé au moment de l’échec. Pour une requête vers la boutique, ce contexte peut inclure le code d’erreur propre au serveur (`serverCode`), le `status` HTTP ou l’`endpoint` ; les champs affichés dépendent de l’endroit où l’échec s’est produit. Pour signaler un problème, utilisez **Copier les infos de débogage** en haut de l’écran Journaux (**Partager les infos de débogage** sur téléphone et tablette) plutôt que des captures d’écran : cela regroupe la version de l’application, l’état de la connexion et les erreurs les plus récentes. Les journaux sont conservés 30 jours au maximum ; collectez-les donc pendant que le problème est récent. Copiez également toute erreur de la console du navigateur apparue avant que le POS n’ait pu écrire sa propre entrée de journal.

Au-delà du journal intégré à l’application, cet échec peut laisser des traces dans :

- **Inspecteur réseau** (web et bureau) : ouvrez les outils de développement — appuyez sur F12 dans le navigateur, ou **Avancé → Afficher/Masquer les outils de développement** dans le menu de l’application de bureau — puis sélectionnez l’onglet **Réseau**. Suivez d’abord les consignes de nouvelle tentative de cette page ; ne reproduisez l’action que lorsque ces étapes indiquent que c’est sans risque. Une requête en échec affiche le statut HTTP et le corps brut de la réponse, y compris les pages d’erreur qui n’atteignent jamais le journal du POS.
- **Journaux POS côté serveur** : dans WP Admin, ouvrez **POS → Réglages → Outils → Journaux**. Cette page consigne les avertissements et les erreurs liés au POS déclenchés sur le serveur lui-même, qui peuvent ne jamais apparaître dans l’application. Une pastille rouge sur le menu signale des erreurs côté serveur non lues.

## Détails {#details}

- **Code :** `SYNC201` (`RECORD_REJECTED`)
- **Gravité :** erreur
- **Introduit dans :** WCPOS 1.10.0
