...
Le serveur a répondu, mais le format de réponse n'est pas celui que le POS attendait. Le serveur devrait renvoyer des données JSON, mais autre chose a été reçue.
...
- **Mauvais type de contenu** — Le serveur envoie du HTML au lieu du JSON
...
Visitez `https://votre site.com/wp-json/` dans votre navigateur :
- Devrait renvoyer des données JSON
- Si vous voyez du HTML ou des erreurs, il y a un problème
- Vérifiez la sortie liée au plugin
