...
1. Confirmez que WCPOS est actif sur le site : WP Admin → Extensions.
2. Testez les deux formes REST prises en charge dans un navigateur : l’adresse /wp-json/ de votre boutique et /index.php?rest_route=/. Ne concluez à un blocage REST que si aucune des deux adresses ne renvoie du JSON.
3. Si aucune des deux formes REST ne fonctionne, demandez à l’administrateur du site si une extension de sécurité, un proxy ou un CDN supprime les en-têtes de requête de WCPOS ou bloque les requêtes REST — les règles de pare-feu et les options de durcissement désactivant l’API REST en sont les coupables habituels.
4. Chez certains hébergeurs, réenregistrer les permaliens (Réglages → Permaliens → Enregistrer) rétablit les routes REST manquantes.
