...
Les données que vous essayez de sauvegarder enfreignent les règles de la base de données. Les contraintes garantissent l'intégrité des données en imposant des règles sur les données pouvant être stockées.
...
- **Valeur hors limites** — Le nombre dépasse les limites autorisées

## Comment résoudre

### 1. Vérifiez les champs obligatoires

Assurez-vous que toutes les données obligatoires sont fournies :
