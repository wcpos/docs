...
WCPOS collecte automatiquement les métriques de performance du serveur à chaque opération de récupération de données (produits, commandes, clients, etc.).

À partir de la **v1.10.0**, la vue agrégée se trouve dans **État de la boutique → Performances** : requêtes effectuées, données transférées et temps de réponse typique sur les dernières 24 heures, ainsi qu’une bande de disponibilité horaire (voir [État de la boutique](/support/store-health#performance)). Deux réserves à la lecture : le chiffre des données transférées est un plancher (les réponses qui ne déclarent pas de taille comptent pour zéro), et l’estimation *~N requêtes par jour* affichée à côté du cadran de synchronisation est un débit nominal dérivé de l’intervalle configuré, et non un plafond ni un décompte réel. La variation aléatoire fait bouger le nombre dans les deux sens, un `304` compte tout de même comme une requête, et la décroissance en veille ne le réduit que pour les préréglages plus rapides que 60 secondes. Les métriques de chaque requête apparaissent dans **État de la boutique → Journaux**.
...
1. Ouvrez **État de la boutique** depuis le tiroir de navigation et consultez l’onglet **Performances** pour la vue sur 24 heures (le temps de réponse typique est le chiffre clé)
2. Effectuez l'opération lente (synchronisation des produits, création d'une commande, etc.)
3. Recherchez l’entrée correspondante dans l’onglet **Journaux**
4. Développez la ligne pour voir les métriques
