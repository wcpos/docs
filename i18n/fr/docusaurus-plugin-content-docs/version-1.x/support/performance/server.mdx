...
La performance du serveur impacte directement la vitesse et la réactivité de votre application WCPOS. Ce guide vous aide à surveiller, diagnostiquer et à optimiser la performance du serveur en utilisant les métriques intégrées et les techniques de résolution de problèmes.

## Compréhension des métriques du serveur

WCPOS collecte automatiquement les métriques de performance du serveur à chaque opération de récupération de données (produits, commandes, clients, etc.). Vous pouvez visualiser ces métriques dans l'écran [Logs](/support/logs).
...
- **`execution_time`** - Temps pris pour compléter l'opération (76,64 millisecondes)
...
```json
// Bons exemples de mise à l'échelle
...
// Mauvais exemples de mise à l'échelle
...
**Solutions :**
- **Mettre à niveau le CPU** - Plus de cœurs gèrent mieux les requêtes simultanées
...
**Dépannage :**
1. **Tester sur un environnement de staging** - Désactiver tous les plugins sauf WooCommerce et WCPOS
...
- **Surveillance durant les périodes de pointe** - Surveiller durant les périodes de fort trafic
...
```php
// Dans wp-config.php - limiter les révisions de publication
define('WP_POST_REVISIONS', 3);

// Activer la journalisation des erreurs de débogage de WordPress si nécessaire
