...
Si vous rencontrez des lenteurs de performance pendant le processus de caisse, ce guide vous aidera à identifier et à résoudre les causes les plus courantes.

## Comment fonctionne la Caisse WCPOS {#how-wcpos-checkout-works}

La caisse WCPOS ouvre une modal avec un iframe/webview qui charge une version allégée du modèle de commande pay WooCommerce. C'est essentiellement la même page que la [page de paiement client](https://woocommerce.com/document/managing-orders/paying-for-orders/#customer-payment-page-link) que les clients voient lors du paiement des commandes en ligne.
...
## Optimisations intégrées {#built-in-optimizations}

WCPOS inclut plusieurs optimisations pour améliorer les performances de la caisse :

- **Modèle allégé** - Supprime les éléments de thème inutiles et l'encombrement
...
### 1. Scripts et Styles Inutiles {#1-unnecessary-scripts-and-styles}
...
### 2. Interférence des Plugins lors de la Création de Commandes {#2-plugin-interference-during-order-creation}
...
2. **Testez la vitesse de la caisse** - Traitez une commande test et notez la performance
3. **Activez les plugins progressivement** - Si la performance s'améliore de manière significative, réactivez les plugins un par un pour identifier le coupable
...
### 3. Limitations des Ressources Serveur {#3-server-resource-limitations}
...
## Meilleures Pratiques pour Développeurs WordPress {#wordpress-developer-best-practices}
