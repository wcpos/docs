...
```html
<div style="font-style: italic; color: #6b7280;">Entreprise familiale depuis 1987</div>
```

Dans un modèle thermique :

```xml
<align mode="center"><text>Entreprise familiale depuis 1987</text></align>
```
...
```html
<!-- Par défaut : code-barres Code 128 du numéro de commande -->
<barcode type="code128">{{order.number}}</barcode>

<!-- Passer à un QR code -->
<barcode type="qrcode">{{order.number}}</barcode>

<!-- Ou encoder autre chose, comme une URL de retours -->
...
La même syntaxe `<barcode>` fonctionne dans les modèles HTML et thermiques. Les autres types pris en charge incluent `ean13`, `ean8`, `upca`, `pdf417` et [tout ce que bwip-js prend en charge](https://github.com/metafloor/bwip-js/wiki/Supported-Barcode-Types).
  ...
  <AccordionItem question="Comment envoyer un autre modèle vers une imprimante spécifique ?">
