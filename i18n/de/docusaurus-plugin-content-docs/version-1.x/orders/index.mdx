---
title: Bestellungen
sidebar_label: Bestellungen
sidebar_position: 1
---

import Image from "@theme/IdealImage";
import Icon from '@site/src/components/Icon';

:::info Pro Feature
Der Bestellbildschirm erfordert [WCPOS Pro](/getting-started/pro-license).
:::

Der Bestellbildschirm bietet Zugriff auf Ihre Bestellhistorie direkt innerhalb des POS. Sehen Sie vergangene Bestellungen an, drucken Sie Quittungen erneut und verwalten Sie Bestelldetails, ohne zum WooCommerce-Admin zu wechseln.

{/* Screenshot: orders-screen.png */}

## Übersicht der Benutzeroberfläche

### Suche & Filter

Am oberen Rand des Bildschirms:

- **Suchleiste** - Finden Sie Bestellungen nach Nummer, Kundenname usw.
- **Statusfilter** - Filtern Sie nach Bestellstatus (Abgeschlossen, In Bearbeitung usw.)
- **Kundenfilter** - Filtern Sie nach Kunde
- **Kassiererfilter** - Filtern Sie nach dem, der die Bestellung bearbeitet hat
- **Geschäftsfilter** - Filtern Sie nach Geschäft (für Multi-Store-Setups)
- **Datumsbereich** - Filtern Sie nach Datum

Aktive Filter erscheinen als blaue Abzeichen, die durch Klicken auf das × entfernt werden können.

### Bestellungen Tabelle

Der Hauptbereich zeigt Bestellungen mit:

- **Statussymbol** - Visueller Indikator (✓ abgeschlossen, 🕐 ausstehend, 🛒 offen)
- **Bestellnummer** - Einmalige Bestell-ID
- **Kunde** - Kundenname (oder "Gast")
- **Rechnungsadresse** - Kundenabrechnungsinformationen
- **Hinweisindikator** - Sprechblase, wenn die Bestellung Hinweise hat
- **Erstellungsdatum** - Relative Zeitstempel ("vor 2 Stunden")
- **Kassierer** - Wer die Bestellung bearbeitet hat
- **Zahlungsmethode** - Bar, Karte usw.
- **Gesamt** - Gesamtbetrag der Bestellung
- **Aktionen** - Drei-Punkte-Menü

### Fußzeile

- Bestellanzahl mit Sync-Button (<Icon name="arrow-rotate-right" />). **Langes Drücken** für die Optionen Löschen und Aktualisieren

## Hauptmerkmale

### Bestellsuche

Finden Sie schnell Bestellungen mithilfe von:

- **Bestellnummernsuche**
- **Kundenname-Suche**
- **Datumsbereich-Filterung**
- **Status-Filterung**
- **Kassierer-Filterung**

### Quittungsneudruck

Für jede Bestellung können Sie:

1. Klicken Sie auf das Drei-Punkte-Menü
2. Wählen Sie die Quittungsoption
3. Quittung ansehen, drucken oder per E-Mail senden

### Bestelldetails

Klicken Sie auf eine Bestellung, um Folgendes anzuzeigen:

- Vollständige Einzelposten-Details
- Kundeninformationen
- Zahlungsdetails
- Bestellhinweise
- Metadaten

## Anzeigeeinstellungen

Klicken Sie auf das Schieberegler-Symbol (<Icon name="sliders" />), um die sichtbaren Spalten anzupassen.

<div class="image-container">
  <Image
    alt="Bestellungseinstellungen"
    img="/img/orders-page-settings.png"
    style={{ maxHeight: 800 }}
  />
  <p class="image-caption">Anzeigeeinstellungen für Bestellungen</p>
</div>

### Verfügbare Spalten

| Spalte | Beschreibung |
|--------|-------------|
| **Status** | Bestellstatussymbol |
| **Bestellnummer** | Einmalige Bestell-ID |
| **Kunde** | Kundenname und Details |
| **Rechnungsadresse** | Abrechnungsinformationen |
| **Lieferadresse** | Versandinformationen |
| **Kundenhinweis** | Hinweise vom Kunde |
| **Erstellungsdatum** | Wann die Bestellung aufgegeben wurde |
| **Bearbeitungsdatum** | Letzte Änderung |
| **Abschlussdatum** | Wann die Bestellung abgeschlossen wurde |
| **Zahlungsdatum** | Wann die Zahlung eingegangen ist |
| **Erstellt über** | Bestellquelle (POS, Online usw.) |
| **Kassierer** | Wer die Bestellung bearbeitet hat |
| **Zahlungsmethode** | Verwendete Zahlungsart |
| **Gesamt** | Gesamtbetrag der Bestellung |
| **Quittung** | Schneller Zugriff auf Quittungen |
| **Aktionen** | Bearbeiten, ansehen usw. |

## Bestellaktionen

Klicken Sie auf das Drei-Punkte-Menü (⋮) für Optionen:

- **Anzeigen** - Vollständige Bestelldetails anzeigen
- **Bearbeiten** - Bestellinformationen ändern
- **Quittung** - Quittung anzeigen/drucken/versenden
- **Synchronisieren** - Bestellung vom Server aktualisieren

## Anwendungsfälle

### Kundenservice

- Vergangene Bestellungen einsehen, um bei Rücksendungen oder Anfragen zu helfen
- Überprüfen, was ein Kunde gekauft hat
- Quittungen auf Anfrage erneut drucken

### Tagesabschluss

- Überprüfen Sie alle Bestellungen des Tages
- Nach Kassierer filtern, um Einzelbeträge zu überprüfen
- Abgleich mit [Berichten](/reports) zur Abstimmung

### Bestellverfolgung

- Bestellstatus überwachen
- Verfolgen, welche Bestellungen ausstehend bzw. abgeschlossen sind
- Bestellungen identifizieren, die Aufmerksamkeit benötigen

## Verwandte Dokumentation

- [Berichte](/reports) - Verkaufsberichte und Abstimmungen
- [Quittungen](/pos/checkout/receipts) - Anpassung von Quittungen

---