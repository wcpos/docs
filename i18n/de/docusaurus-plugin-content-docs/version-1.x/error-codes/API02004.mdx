...
# API02004: Benutzer Nicht Autorisiert {#api02004-user-not-authorized}
...
## Zuordnung des Serverfehlers {#server-error-mapping}
...
### 1. Überprüfen Sie die Benutzerrolle {#1-check-user-role}

In WordPress Admin → Benutzer:
1. Finden Sie Ihr Benutzerkonto
2. Überprüfen Sie die Rolle (z.B. Shop-Manager, Administrator)
3. Stellen Sie sicher, dass die Rolle WooCommerce-Fähigkeiten umfasst

### 2. Aktivieren Sie den POS-Zugang {#2-enable-pos-access}
...
### 3. Zusätzliche Berechtigungen Anfordern {#3-request-additional-permissions}
...
### 4. Überprüfen Sie die Ladenzuweisung {#4-check-store-assignment}
