...
sidebar_position: 1
description: Einen Bondrucker mit WCPOS verbinden — über Netzwerk, Bluetooth oder USB, in der Web-, Desktop- und mobilen App.
---
...
## Connection Types (Mobile Apps) {#connection-types-mobile-apps}
...
### USB (Android only) {#usb-android-only}
...
Wenn Sie mehr als eine Vorlage verwenden — zum Beispiel einen Thermobeleg **und** eine A4-Rechnung — legt die Druckweiterleitung fest, auf welchem Drucker jede Vorlage ausgegeben wird. Die Weiterleitung hat drei Ebenen, die in dieser Reihenfolge geprüft werden:

1. **Override pro Druckauftrag.** Auf dem Belegbildschirm befindet sich neben dem Vorlagenumschalter ein Drucker-Dropdown. Die Auswahl eines Druckers hier überschreibt alles nur für diesen einen Druckauftrag. Beim Wechsel der Vorlage wird die Auswahl wieder auf **Automatisch** zurückgesetzt.
2. **Override in den Einstellungen.** Gehen Sie zu **Einstellungen → Druckweiterleitung**, um jeder Vorlage einen bestimmten Drucker zuzuweisen. Leiten Sie zum Beispiel den Thermobeleg an den Epson-Netzwerkdrucker und die HTML-Rechnung an den Systemdruckdialog weiter. Stellen Sie eine Vorlage wieder auf **Automatisch**, um den Override zu entfernen.
3. **Automatische Zuordnung.** Wenn kein Override gesetzt ist, ordnet WCPOS automatisch zu:
   - **Thermovorlagen** werden an Thermodrucker weitergeleitet, deren Zeichenbreite passt. Eine 58-mm-Thermovorlage bevorzugt Drucker mit 32 Spalten; eine 80-mm-Vorlage bevorzugt Drucker mit 42 oder 48 Spalten.
   - **HTML-Vorlagen** werden an den Systemdruckdialog weitergeleitet.
   - Wenn mehrere Drucker passen, gewinnt der **Standarddrucker**.

Wenn Sie eine Vorlage manuell an einen inkompatiblen Drucker senden — zum Beispiel eine Thermovorlage an den Systemdialog — erscheint auf dem Belegbildschirm eine gelbe **Warnung wegen Nichtübereinstimmung**. Der Druck wird trotzdem fortgesetzt, aber die Ausgabe wird möglicherweise nicht korrekt dargestellt.

Weiterleitungs-Overrides werden **pro Gerät** gespeichert. Jedes iPad, Telefon oder jeder Computer verwaltet seine eigene Weiterleitung — es gibt keine serverseitige Synchronisierung.

:::note
Die Seite Druckweiterleitung erscheint erst, nachdem mindestens ein Drucker hinzugefügt wurde. Wenn keine Drucker konfiguriert sind, verwendet jede Vorlage implizit den Systemdruckdialog.
:::
