...
WCPOS sammelt automatisch Serverleistungsmetriken bei jedem Datenabrufvorgang (Produkte, Bestellungen, Kunden usw.).

Ab **v1.10.0** befindet sich die Gesamtansicht unter **Shop-Zustand → Leistung**: gestellte Anfragen, übertragene Datenmenge und typische Antwortzeit der letzten 24 Stunden sowie ein stündlicher Verfügbarkeitsstreifen (siehe [Shop-Zustand](/support/store-health#performance)). Zwei Einschränkungen beim Lesen: Der Wert der übertragenen Datenmenge ist eine Untergrenze (Antworten, die keine Größe angeben, zählen mit null), und die Angabe *~N Anfragen pro Tag* neben dem Sync-Regler ist eine nominelle Rate, die aus dem eingestellten Intervall abgeleitet wird — weder eine Obergrenze noch ein tatsächlicher Zählwert. Die Streuung verschiebt den Wert in beide Richtungen, ein `304` zählt trotzdem als Anfrage, und die Verlangsamung im Leerlauf senkt ihn nur bei Voreinstellungen, die schneller als 60 Sekunden sind. Einzelne Anfragemetriken erscheinen unter **Shop-Zustand → Protokolle**.
...
1. Öffnen Sie **Shop-Zustand** aus dem Navigationsbereich und rufen Sie den Tab **Leistung** für das 24-Stunden-Bild auf (die typische Antwortzeit ist die entscheidende Zahl)
2. Führen Sie den langsamen Vorgang aus (Produkte synchronisieren, Bestellung erstellen usw.)
3. Suchen Sie den entsprechenden Eintrag im Tab **Protokolle**
4. Erweitern Sie die Zeile, um die Metriken anzuzeigen
