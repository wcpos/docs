...
- **Mantenga exactos los nombres de marcas y productos.** `WCPOS`, `WCPOS Pro`, `WooCommerce`, `WordPress`, `Stripe`, `Stripe Terminal` y `SumUp` se mantienen tal cual. Use **WCPOS** en el texto dirigido a clientes; los nombres de producto antiguos solo corresponden a identificadores técnicos como slugs y URL.
