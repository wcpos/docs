...
WCPOS recopila automáticamente métricas de rendimiento del servidor con cada operación de obtención de datos (productos, pedidos, clientes, etc.).

A partir de la **v1.10.0**, la vista agregada se encuentra en **Estado de la tienda → Rendimiento**: solicitudes realizadas, datos transferidos y tiempo de respuesta típico de las últimas 24 horas, además de una franja de disponibilidad por horas (consulte [Estado de la tienda](/support/store-health#performance)). Dos advertencias al leerla: la cifra de datos transferidos es un límite inferior (las respuestas que no declaran un tamaño aportan cero) y la estimación de *~N solicitudes al día* que aparece junto al control de sincronización es una tasa nominal derivada del intervalo configurado, no un tope ni un recuento real. La variación aleatoria mueve el recuento en ambos sentidos, un `304` sigue contando como una solicitud y el decaimiento por inactividad solo lo reduce en los preajustes más rápidos que 60 segundos. Las métricas de cada solicitud individual aparecen en **Estado de la tienda → Registros**.
...
1. Abra **Estado de la tienda** desde el panel de navegación y consulte la pestaña **Rendimiento** para tener la visión de las últimas 24 horas (el tiempo de respuesta típico es la cifra clave)
2. Realice la operación lenta (sincronizar productos, crear pedido, etc.)
3. Busque la entrada correspondiente en la pestaña **Registros**
4. Expanda la fila para ver las métricas
