...
WCPOS recopila automáticamente métricas de rendimiento del servidor con cada operación de recuperación de datos (productos, pedidos, clientes, etc.). Puede ver estas métricas en la pantalla de [Logs](/support/logs).
...
|-----------|-------|-----------|-------|---------|
| **Búsqueda de productos** | < 100ms | 100-500ms | 500ms-2s | > 2s |
...
### Paso 1: Monitorear los Logs

1. Abra **Logs** desde el panel de navegación
2. Realice la operación lenta (sincronizar productos, crear pedido, etc.)
3. Busque la entrada de log correspondiente
...
- Códigos de error relacionados con la base de datos en los logs
...
- Errores relacionados con la memoria en los logs
...
- **Revisiones semanales** - Verificar logs para tendencias de rendimiento
...
- Frecuentes errores de tiempo de espera en los logs
...
```php
// In wp-config.php - limite de revisiones de post
define('WP_POST_REVISIONS', 3);

// Habilitar el registro de depuración de WordPress si es necesario
...
- **Errores de memoria** apareciendo frecuentemente en los logs
- **Consultas de base de datos que tardan > 2 segundos** consistentemente

Proporcióneles:
- Métricas del servidor de sus logs
...
- [Logs](/support/logs) - Cómo acceder e interpretar métricas del servidor
