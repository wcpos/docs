...
El checkout de WCPOS abre un modal con un iframe/webview que carga una versión simplificada de la plantilla de Pago de Pedidos de WooCommerce. Esta es esencialmente la misma página que los clientes ven cuando pagan por pedidos en línea, la [página de Pago del Cliente](https://woocommerce.com/document/managing-orders/paying-for-orders/#customer-payment-page-link).
...
- **Plantilla simplificada** - Elimina elementos de tema innecesarios y desorden
...
Sin embargo, algunos scripts y estilos no se pueden desactivar por completo porque las pasarelas de pago pueden necesitarlos para funcionar correctamente.
...
No desactive scripts que su pasarela de pago requiere. Si un método de pago deja de funcionar después de desactivar un script, vuelva a habilitarlo.
:::
...
3. **Habilite plugins gradualmente** - Si el rendimiento mejora drásticamente, vuelva a habilitar los plugins uno por uno para identificar el culpable
...
2. **Optimice los recursos del servidor:**
   ...
   - Use plugins de caché (pero pruebe la compatibilidad)
