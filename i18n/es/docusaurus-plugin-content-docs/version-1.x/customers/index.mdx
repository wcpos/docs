---
title: Clientes
sidebar_label: Clientes
sidebar_position: 1
description: "Gestión de clientes de WooCommerce en WCPOS, incluyendo búsqueda de clientes, edición de datos y consulta del historial de pedidos."
---

import Image from "@theme/IdealImage";
import Icon from '@site/src/components/Icon';
import LinkCards from '@site/src/components/LinkCards';
import LinkCard from '@site/src/components/LinkCard';

:::info Función Pro
La pantalla de Clientes requiere [WCPOS Pro](/getting-started/pro-license). Los usuarios de la versión gratuita pueden seleccionar clientes existentes para los pedidos, pero no pueden ver la lista completa de clientes ni editar los datos de los clientes.
:::

La pantalla de Clientes ofrece una gestión integral de clientes directamente dentro del POS. Es posible ver, editar y crear clientes sin necesidad de cambiar al panel de administración de WooCommerce.

{/* Screenshot: customers-screen.png */}

## Descripción general de la interfaz {#interface-overview}

### Acciones del encabezado {#header-actions}

En la parte superior de la pantalla:

- **Barra de búsqueda** - Permite buscar clientes por nombre, correo electrónico, etc.
- **Añadir cliente** (<Icon name="plus" />) - Crear un nuevo cliente
- **Configuración de visualización** (<Icon name="sliders" />) - Configurar las columnas visibles

### Tabla de clientes {#customer-table}

El área principal muestra los clientes con:

- **Avatar** - Foto de perfil del cliente (o marcador de posición)
- **Nombre** - Nombre del cliente
- **Apellido** - Apellido del cliente (ordenable)
- **Correo electrónico** - Dirección de correo electrónico de contacto
- **Dirección de facturación** - Dirección de facturación completa
- **Fecha de creación** - Fecha en que se añadió el cliente
- **Acciones** - Menú de tres puntos

### Pie de página {#footer}

- Contador de clientes con botón de sincronización (<Icon name="arrow-rotate-right" />). **Pulsación larga** para la opción de Limpiar y Actualizar

## Características principales {#key-features}

### Búsqueda de clientes {#customer-search}

Es posible encontrar clientes rápidamente por:

- Nombre o apellido
- Dirección de correo electrónico
- Información de dirección

La búsqueda encuentra **todas las palabras** que introduzca, en cualquier orden y en esos campos; por ejemplo, "jane paris" encuentra a Jane, cuya ciudad es París. La ordenación y el filtrado se realizan en el servidor y abarcan a todos sus clientes, no solo a los que ya están en el dispositivo.

### Agregar nuevo cliente {#add-new-customer}

Es posible crear clientes directamente desde el POS:

1. Hacer clic en el <Icon name="plus" /> icono en el encabezado
2. Completar los datos del cliente
3. Guardar el nuevo cliente

El cliente se crea en WooCommerce y queda disponible de inmediato para los pedidos.

### Editar cliente {#edit-customer}

Actualizar la información del cliente:

1. Hacer clic en el menú de tres puntos de un cliente
2. Seleccionar **Editar**
3. Modificar los datos (nombre, correo electrónico, direcciones)
4. Guardar los cambios

Los cambios se sincronizan con WooCommerce automáticamente.

## Ajustes de visualización {#display-settings}

Haga clic en el icono de controles deslizantes (<Icon name="sliders" />) para personalizar las columnas visibles.

<div class="image-container">
  <Image
    alt="Ajustes de clientes"
    img="/img/customers-page-settings.png"
    style={{ maxHeight: 800 }}
  />
  <p class="image-caption">Ajustes de visualización de clientes</p>
</div>

### Columnas disponibles {#available-columns}

| Columna | Descripción |
|--------|-------------|
| **Imagen** | Avatar del cliente |
| **ID** | ID de cliente de WooCommerce |
| **Nombre** | Nombre del cliente |
| **Apellido** | Apellido del cliente |
| **Correo electrónico** | Dirección de correo electrónico |
| **Rol** | Rol de usuario (Cliente, Suscriptor, etc.) |
| **Nombre de usuario** | Nombre de usuario de WordPress |
| **Dirección de facturación** | Información de facturación |
| **Dirección de envío** | Información de envío |
| **Fecha de creación** | Fecha en que se añadió el cliente |
| **Fecha de modificación** | Última actualización |
| **Acciones** | Editar, Sincronizar, Eliminar |

## Acciones de cliente {#customer-actions}

Hacer clic en el menú de tres puntos (⋮) para ver las opciones:

- **Editar** - Modificar los datos del cliente
- **Sincronizar** - Actualizar los datos del cliente desde el servidor
- **Eliminar** - Eliminar de la base de datos local

:::note
Al eliminar un cliente del POS, solo se elimina **localmente**: el cliente permanece en WooCommerce y vuelve a aparecer en la siguiente sincronización. Si se elimina un cliente **en WooCommerce**, la sincronización también lo elimina del POS.
:::

## Cliente predeterminado (Invitado) {#default-customer-guest}

Cuando no se selecciona ningún cliente, los pedidos se realizan como pedidos de **Invitado**. Los pedidos de invitado:

- No tienen nombre de cliente, correo electrónico ni dirección asociados
- No se pueden buscar por cliente en el historial de pedidos
- Siguen siendo visibles en la pantalla de Pedidos y en el administrador de WooCommerce

Para evitar pedidos de invitado, es necesario seleccionar un cliente desde el Panel del carrito antes de finalizar la compra. Si el negocio requiere un cliente en cada pedido, se recomienda capacitar a los cajeros para que siempre asignen uno; no existe una configuración integrada para imponer este requisito.

## Uso de clientes en pedidos {#using-customers-in-orders}

Para asignar un cliente a un pedido:

1. En el [Panel del carrito](/pos/cart), hacer clic en la insignia del cliente
2. Buscar el cliente por nombre, correo electrónico o número de teléfono
3. Seleccionar el cliente
4. El cliente ahora está asociado con el pedido

La información del cliente (direcciones de facturación/envío) se utiliza automáticamente para el pedido.

## Sincronización {#synchronization}

Los datos del cliente se sincronizan entre el POS y WooCommerce:

- **Sincronizados entre dispositivos** - Los cambios aparecen en los demás dispositivos en su siguiente comprobación de cambios
- **Integración con WooCommerce** - Los clientes se almacenan en WooCommerce
- **Capacidad sin conexión** - Consultar clientes incluso sin conexión a internet

## Documentación relacionada {#related-documentation}

<LinkCards>
  <LinkCard to="/pos/cart" title="Panel del carrito" description="Selección de clientes para pedidos" />
  <LinkCard to="/orders" title="Pedidos" description="Ver el historial de pedidos del cliente" />
</LinkCards>
