...
# API04006: Recurso No Encontrado {#api04006-resource-not-found}
...
### 1. Verificar si el Recurso Existe {#1-check-if-the-resource-exists}
...
### 2. Actualizar Datos Locales {#2-refresh-local-data}

Si el recurso fue eliminado del lado del servidor:
1. Abre la configuración del POS
2. Navega a la sección de datos relevante
3. Inicie una sincronización/actualización para actualizar los datos locales
4. El elemento eliminado debería ser eliminado localmente

### 3. Verificar los Permalinks de WordPress {#3-check-wordpress-permalinks}
...
### 4. Verificar Rutas de la API {#4-verify-api-routes}
...
### 5. Verificar Conflictos de Plugins {#5-check-for-plugin-conflicts}
