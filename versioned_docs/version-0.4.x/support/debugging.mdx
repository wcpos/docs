---
title: Debugging
description: If debugging is enabled, WCPOS will log various messages to the console which may assist in diagnosing problems.
---
...
WCPOS is largely a JavaScript web application - and as such - most debugging is done through the [browser console](http://codex.wordpress.org/Using_Your_Browser_to_Diagnose_JavaScript_Errors).
If debugging is enabled, WCPOS will log various messages to the console which may assist in diagnosing problems.
...
### Turn on debugging using wp-config.php {#turn-on-debugging-using-wp-configphp}

By setting the [SCRIPT_DEBUG](https://codex.wordpress.org/Debugging_in_WordPress#SCRIPT_DEBUG) flag in your wp-config.php file, you can enable WCPOS debugging.
This will load the unminified script files and also enable debug messages in the browser console.

### Turn on debugging using the browser console {#turn-on-debugging-using-the-browser-console}
