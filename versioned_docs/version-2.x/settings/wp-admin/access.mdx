...
description: "Configure user roles and capabilities for WCPOS access including Administrator, Shop Manager, and Cashier permissions."
---
...
| Cashier       | WCPOS | This role is added by the WCPOS plugin and is intended for users who will primarily use the POS. Cashiers can run the till and, **by default, create and edit products and customers** — but they don't have access to WP Admin to modify POS settings, install plugins, or create blog posts. You can tighten or loosen any of this per capability (see below). |

:::note Cashiers can create and edit products by default
...
:::note Customers, staff and the "Manage customers" toggle
...
## Partly granted tasks and restoring the defaults {#restore-defaults}

Each task toggle on the Access screen stands for a set of capabilities. When a role holds only some of them, the box shows a dash instead of a tick and a badge such as **3 of 7** appears next to the task name. Hover the badge to see what it means, or click **Review** to open the **Advanced** list and see which individual capabilities are missing. Ticking a dashed box grants the rest of the set; unticking it removes the whole set.

The screen also shows whether the selected role still matches what WCPOS grants on a fresh install. A role that has been changed carries an amber dot in the role list and the line **Changed from the WCPOS defaults** under its heading. **Restore defaults** puts the role back. Before anything is written, a dialog lists which tasks will be granted, which will be removed and which stay granted, and names any individual capability under **Advanced** that the restore would change. For the Cashier role the defaults are the full set WCPOS installs. For Administrator and Shop Manager they are only the POS capabilities, because WCPOS does not own those roles' WooCommerce or WordPress permissions. For any other role the default is no POS access, and the dialog says so. A restore writes only the capabilities the Access screen lists for that role; anything else on the role is left alone, but a listed capability is reset even if another plugin granted it.

:::note Changes made on the Access screen used to be lost on update
Plugin updates up to and including 1.10.17 re-applied the default grants to the Administrator, Shop Manager and Cashier roles on every update. A store that had switched off **Create & edit products** for cashiers found it switched on again after each update, which is why some stores saw cashier permissions change after updating. This is fixed in the first release after 1.10.17: an update grants only capabilities that are new in that release, so a capability you removed stays removed. Deactivating and re-activating the plugin still resets every role to the defaults; that is the documented repair, and an update does not trigger it.
:::
...
| --------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `access_woocommerce_pos`    | Allows access to the WCPOS interface.                        | Enabling this option would grant the cashier permission to access and use the POS, allowing them to perform point-of-sale transactions.          |
| `manage_woocommerce_pos`    | Enables the management of WCPOS settings and configurations. | Enabling this option would give the cashier the ability to modify settings related to the POS, such as payment methods or receipt templates.     |
...
## What each default role can do, at a glance {#what-each-default-role-can-do-at-a-glance}
...
For the full per-endpoint breakdown — including the Pro plugin endpoints, token-expiry behaviour, and tips for distinguishing capability failures from token failures — see the [Role Endpoint Access reference](/reference/role-endpoint-access).

## A user with more than one role {#more-than-one-role}

WordPress lets one account hold several roles, although the profile screen only shows this when a plugin such as [Members](https://wordpress.org/plugins/members/) adds the role checkboxes. WordPress merges the capabilities of every role the account holds, so a second role on its own rarely takes anything away. Role-editor plugins change that. Members ships with **Denied capabilities should always overrule granted capabilities** switched on, so a capability marked *Deny* on **any** of the account's roles wins over a grant from every other role, whatever order the roles are in. An Administrator who is also a Customer inherits every *Deny* placed on the Customer role.
