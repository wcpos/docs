...
3. On Cloudflare, add a WAF **Skip** rule for the REST API paths; Bot Fight Mode (Free plan) cannot be bypassed by any rule and must be turned off. The [Cloudflare guide](/support/troubleshooting/cloudflare) has the exact rule and a `curl` check.
