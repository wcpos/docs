...
### 1. Check URL Format {#1-check-url-format}
...
### 2. Include the Protocol {#2-include-the-protocol}

Always include `https://` or `http://`:
- Preferred: `https://` (secure)
- Fallback: `http://` (if HTTPS isn't available)

### 3. Remove Extra Characters {#3-remove-extra-characters}
...
### 4. Verify the URL Works {#4-verify-the-url-works}
...
### 5. Update POS Configuration {#5-update-pos-configuration}
