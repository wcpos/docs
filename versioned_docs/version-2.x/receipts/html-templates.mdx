...
Use `_display` variants for pre-formatted currency values (e.g., `$12.50` instead of `12.5`). See the [Receipt Data Reference](/receipts/receipt-data) for all available fields.
...
Use the `<barcode>` element — the same syntax as [thermal templates](/receipts/thermal-templates). The value goes inside the element; the symbology is set on the `type` attribute. The renderer replaces each element with an inline SVG.
