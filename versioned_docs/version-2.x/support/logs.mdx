...
:::note What the log is for
The log is a 30-day working record on this device, not the record of a sale. Every payment taken at the till is kept on the order itself in WooCommerce, with its own short event history (the most recent 100 events per payment). If you need to resolve a dispute about a payment, start from the order in WooCommerce → Orders and its payment record, then use the log for what happened on this device around it.
:::
