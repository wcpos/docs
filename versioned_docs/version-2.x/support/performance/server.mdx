...
WCPOS automatically collects server performance metrics with each data fetch operation (products, orders, customers, etc.).

From **v1.10.0**, the aggregate view lives in **Store health → Performance**: requests made, data transferred, and typical response time for the last 24 hours, plus an hourly uptime strip (see [Store Health](/support/store-health#performance)). Two caveats when reading it: the data-transferred figure is a floor (responses that don't declare a size contribute zero), and the *~N requests per day* estimate beside the sync dial is a nominal rate derived from the configured interval, not a ceiling or actual count. Jitter moves the count in either direction, a `304` still counts as a request, and idle decay lowers it only for presets faster than 60 seconds. Individual request metrics appear in **Store health → Logs**.
...
1. Open **Store health** from the navigation drawer and check the **Performance** tab for the 24-hour picture (typical response time is the key number)
2. Perform the slow operation (sync products, create order, etc.)
3. Look for the corresponding entry in the **Logs** tab
4. Expand the row to see metrics
...
| **Cloudflare** | Security checks (Bot Fight Mode, Under Attack Mode, WAF challenges) block REST API requests, and the cache may serve stale API responses | Follow the [Cloudflare guide](/support/troubleshooting/cloudflare): add a WAF **Skip** rule for `/wp-json/` and a cache rule to **bypass cache** for the same paths |
