...
- **An idle check is low-cost.** The engine sends conditional requests, so a no-change check is answered by a single bodyless `304 Not Modified` — still one request, but with no payload and minimal server work.
...
:::note The in-app "requests per day" figure is nominal
