...
sidebar_position: 1
description: View and manage WooCommerce orders in WCPOS, including order history, refunds, and reprinting receipts.
---
...
1. Find the order in the Orders screen
2. Click the three-dot menu and select **Edit**
3. The order reopens in the cart where adjustments can be made

For full refunds, process the refund through `WP Admin > WooCommerce > Orders` where WooCommerce provides the standard refund interface including partial refund amounts and restocking options.

:::note
If the order was paid via a gateway that supports automatic refunds (e.g., Stripe Terminal), WooCommerce can process the refund back to the original payment method. For cash or manual card payments, the refund is recorded in WooCommerce but the actual money handling is done by the cashier.
:::

## Use Cases

<UseCases>
  <UseCaseCard icon="users" title="Customer Service">
  ...
  <UseCaseCard icon="clock" title="End of Day">
  ...
  <UseCaseCard icon="chart-line" title="Order Tracking">
...
<LinkCards>
  <LinkCard to="/reports" title="Reports" description="Sales reports and reconciliation" />
  <LinkCard to="/pos/checkout/receipts" title="Receipts" description="Receipt customisation" />
</LinkCards>
