...
2. Set **Mode** to `Test` for a simulated test-terminal payment, or `Live` for a real terminal
3. Set **API key source** to reuse the key from the official Mollie plugin or use the key entered on this screen
4. If you enter the key here, paste the test or live API key that matches the selected mode into **Mollie API Key**
5. Save

:::note Test before going live
Mollie provides a simulated terminal for test `pointofsale` payments. Use `Live` mode only when you are ready to send payments to a physical or iOS/Android terminal. See [Scope & Limitations](#scope-and-limitations) for the current test workflow.
:::
...
| **Mode** | `Test` uses Mollie's simulated test terminal; `Live` sends payments to a real terminal |
...
<RequirementsList>
  <RequirementItem label="Mollie Account">Active Mollie account with an API key for the selected mode</RequirementItem>
  <RequirementItem label="Compatible Hardware">An active Mollie Terminal for live payments; Mollie's simulated test terminal for testing</RequirementItem>
...
:::important Test mode
Mollie supports test `pointofsale` payments without a physical terminal. Enable Point of Sale on a Mollie profile to create its test terminal, then set this gateway to `Test`, use the matching test API key, and select that terminal. Mollie returns a `changePaymentState` URL for choosing the simulated result.

The plugin can create and poll the test payment, but its checkout panel does not currently display that URL. In your browser's developer tools, find the `mtfwc_start_payment` network response and open `_links.changePaymentState.href` to set the result, then let the POS poll and reconcile it. See [Mollie's point-of-sale testing guide](https://docs.mollie.com/docs/in-person-payments-testing). Use `Live` mode for a physical or iOS/Android terminal.
:::
...
<Accordion>
  ...
  - Confirm **Mode** and the effective API key use the same Mollie environment
  - In Test mode, enable Point of Sale on a Mollie profile so Mollie creates its simulated test terminal
  - In Live mode, check the terminal is registered and **active** on your Mollie account; inactive terminals are hidden because Mollie cannot reactivate them
