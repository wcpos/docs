...
Install from `WP Admin > POS > Settings > Extensions`, or download the latest **plugin zip asset** (not the GitHub source-code zip or tarball) from the [GitHub releases page](https://github.com/wcpos/mollie-terminal-for-woocommerce/releases) and upload it via `Plugins > Add New > Upload Plugin`.
  ...
  <StepCard number="2" title="Enter your Mollie API key">

1. Go to `WP Admin > WooCommerce > Settings > Payments` and open **Mollie Terminal**
2. Set **Mode** to `Live`
3. Paste your Mollie **live API key** into **Mollie API Key**
4. Save

:::warning Use your live API key
Mollie terminals only exist on **live** accounts. The Mollie **test** API key cannot drive a terminal, so leave Mode on `Live` for real payments — see [Scope & Limitations](#scope-and-limitations).
:::
...
1. Pick a **Default terminal** from the dropdown. The list is fetched live from your Mollie account — inactive terminals are hidden, because Mollie cannot reactivate them.
2. *(Optional)* Restrict **Enabled terminals** to the devices actually in use. Only those can be chosen at checkout; leave it empty to allow all active terminals.
...
| **Mode** | `Test` or `Live`. Terminal payments require **Live** — see below |
| **Mollie API Key** | Your Mollie live API key. The only credential the plugin needs |
| **Default terminal** | The terminal used by default at checkout, chosen from a live dropdown of your active terminals |
| **Enabled terminals** | Restricts the checkout list to selected terminals. Empty = all active terminals |
...
Mollie terminals only exist on **live** accounts. The Mollie **test** API key cannot drive a physical (or iOS/Android) terminal, so terminal payments cannot be exercised end-to-end in the test environment. This is a Mollie platform limitation, not a plugin restriction. The settings screen shows a warning when Test mode is selected.
:::
...
<Accordion>
  ...
  - Confirm **Mode** is set to `Live` and the **Mollie API Key** is your live key — terminals do not exist on test accounts
  ...
  - The plugin cancels open payments automatically on timeout (5 minutes by default); no action is needed
  - If the payment already reached the terminal, cancel it on the device itself — Mollie will not let the plugin cancel it at that point
