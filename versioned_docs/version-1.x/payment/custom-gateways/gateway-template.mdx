...
Copy the generated folder into `wp-content/plugins/`, or zip it and upload through **WP Admin > Plugins > Add New > Upload Plugin**.
...
The template includes a GitHub Actions workflow that automates releases:

1. Open your plugin file and update the `Version:` number in the header (e.g., `0.1.0` to `0.2.0`)
2. Commit and push to the `main` branch
3. GitHub Actions detects the version change and creates a new release with a downloadable ZIP

Users can then download the ZIP from your repository's **Releases** page and install it as a regular WordPress plugin.
