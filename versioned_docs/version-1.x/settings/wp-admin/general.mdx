...
:::note Changing this setting rebuilds the barcode index
When you change the barcode field, WCPOS rebuilds the local catalogue's barcode index so scans match the newly selected field — you don't need to clear and re-download your products by hand.

If scans still aren't matching, you can force a full refresh: go to the Products screen, **long press** the sync icon (↻) at the bottom, and choose **"Clear and Refresh"**. Then test a scan to confirm the new field is being used.
:::
