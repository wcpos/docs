...
As of v1.11, the screen also shows whether the selected role still matches what WCPOS grants on a fresh install. A role that has been changed carries an amber dot in the role list and the line **Changed from the WCPOS defaults** under its heading. **Restore defaults** puts the role back. Before anything is written, a dialog lists which tasks will be granted, which will be removed and which stay granted, and names any individual capability under **Advanced** that the restore would change. For the Cashier role the defaults are the full set WCPOS installs. For Administrator and Shop Manager they are only the POS capabilities, because WCPOS does not own those roles' WooCommerce or WordPress permissions. For any other role the default is no POS access, and the dialog says so. Permissions added to a role by other plugins are left alone.

:::note Changes made on the Access screen used to be lost on update
Plugin updates released before the fix in September 2026 re-applied the default grants to the Administrator, Shop Manager and Cashier roles on every update. A store that had switched off **Create & edit products** for cashiers found it switched on again after each update, which is why some stores saw cashier permissions change after updating. Current versions grant only capabilities that are new in that release, so a capability you removed stays removed. Deactivating and re-activating the plugin still resets every role to the defaults; that is the documented repair, and it is not triggered by an update.
:::
