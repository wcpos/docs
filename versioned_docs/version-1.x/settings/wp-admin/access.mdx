...
## Partly granted tasks {#partly-granted}

Each task toggle on the Access screen stands for a set of capabilities. When a role holds only some of them, the task is marked **partly granted**. Open **Advanced** to see which individual capabilities are missing. Ticking the box grants the rest of the set; unticking it removes the whole set.
