...
These settings cover different stock safeguards, but both can prevent an out-of-stock item from being added:

- **Prevent overselling** (this page) checks available quantities when items are added or updated and validates stock again at checkout.
- **Show out of stock products** (a display setting in the POS product panel) controls how out-of-stock products and variations can be selected. When enabled, out-of-stock products remain visible but can't be added to the cart. When disabled, they are hidden; out-of-stock variation options also grey out, and matching barcode scans are refused.

Turning off Prevent overselling does not override those product-panel rules. You can keep out-of-stock items visible while Prevent overselling adds quantity and checkout safeguards — the cashier sees the item with an *Out of Stock* badge and a disabled add-to-cart.
