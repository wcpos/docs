...
```mermaid
flowchart TD
    Start([First Load]) --> Initial[Initial Sync\nDownload all data]
    Initial --> DB[(Local IndexedDB)]
    DB --> Use[Normal POS Usage]
    Use --> Inc[Incremental Sync\nChanges since last sync]
    Use --> Bg[Background Sync\nPeriodic checks]
    Use --> Man[Manual Sync\nUser-triggered]
    Inc --> DB
    Bg --> DB
    Man --> DB
...
```mermaid
flowchart LR
    subgraph POS["POS Client"]
        Modal[Checkout Modal] --> IFrame[iframe / webview]
    end
    IFrame -->|Loads| OrderPay
    subgraph WC["WooCommerce Server"]
        OrderPay[Order Pay Page] --> Gateway[Payment Gateway]
        Gateway --> Processing[Order Processing]
    end
```
