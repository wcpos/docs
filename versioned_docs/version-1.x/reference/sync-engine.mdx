...
Switching scope is **transactional**: the new store or cashier hydrates its data plane and only commits once it's ready, and any error is surfaced instead of leaving you half-switched between two stores.
...
- **An order write conflict recovers itself once.** If an order write collides with a newer server copy, the engine reconciles and retries a single time; if it still can't, the conflict is surfaced for you to resolve rather than looping silently.
...
If the local store becomes **unreadable**, WCPOS blocks checkout and prompts for a reload rather than letting a sale run against broken data and lose it, and barcode scans in that state fail loudly instead of going quiet — so a corrupted store can't silently swallow a sale before the recovery tooling rebuilds it.
