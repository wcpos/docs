...
The line item's underscore-prefixed meta (including `_woocommerce_pos_data`) is filtered out of `lines[].meta` by `Receipt_Data_Builder::get_item_meta_pairs()`. Receipt Data v1.1 instead exposes the recorded regular price, pre-coupon selling price, and savings through canonical fields such as `lines[].regular_price_display`, `lines[].selling_price_display`, and `lines[].unit_savings_display`. See [Displaying regular price and savings](/receipts/receipt-data#displaying-regular-price-and-savings) for template examples.
