...
- **Measured actuals** — requests made, data transferred, and typical response time over the last 24 hours. These are measurements from this till, not estimates.
...
- The *~N requests per day* figure shown beside the dial is a worst-case ceiling, not a prediction — quiet registers cost less, often several times less, because no-change checks are answered with a single tiny `304` response.
