...
- **Trend charts** — request volume and response time are plotted over time, so you can see whether load is steady, climbing, or spiking rather than reading a single number.
...
At the top, four tiles: the **Ready to sell** milestone, records on this device, storage used, and changes waiting to send. The record counts include **variation totals** alongside parent products, and **storage used** is real device usage, not an estimate. WCPOS also compares this device's clock against the server's and flags **clock skew** here when the two drift far enough apart to affect sync or timestamps.
...
Each row also offers **Check for changes now**, which runs an immediate change check for just that collection instead of waiting for the next scheduled poll — handy after you've changed something on the server and want to confirm it reached this device.
...
- **Infinite scroll** — the ledger loads more rows as you scroll, so you can page back through the full 30-day history without a "next page" button.
