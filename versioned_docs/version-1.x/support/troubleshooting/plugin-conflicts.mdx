...
**WooCommerce Tax (automated taxes via TaxJar):** Supported. WCPOS asks it for rates when an open POS order saves, so POS orders get the same automated rates as online orders. See [WooCommerce Tax](/integrations/woocommerce-tax).
...
| **Stripe Tax** | Incompatible with REST API-created orders (how the POS creates orders) | No workaround — incompatible. [WooCommerce Tax](/integrations/woocommerce-tax) is the supported automated-tax option |
