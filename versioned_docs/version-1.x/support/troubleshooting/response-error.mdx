...
description: "Fix the \"Cannot read properties of undefined\" error in WCPOS caused by empty server responses or plugin conflicts."
---
...
- **Outdated software versions:** Using outdated versions of WordPress, plugins, or themes can lead to compatibility issues and unexpected behaviour. Please make sure you have the [minimum requirements](/getting-started/installation).
...
### 1. Check the Network Tab in the Browser {#1-check-the-network-tab-in-the-browser}
...
### 2. Check the WooCommerce Fatal Error Logs {#2-check-the-woocommerce-fatal-error-logs}
...
### 3. Check the POS Logs {#3-check-the-pos-logs}

The POS activity log in [**Store health → Logs**](/support/logs) may also contain relevant error information and error codes that can help identify the issue.
