...
WCPOS is translated into a dozen-plus languages and there's room for many more. Contributions are very welcome — you don't have to translate everything, and you don't have to be a developer. This page covers what to translate and where.
...
- **Regional fallback** — `pt_BR` falls back to `pt`, `es_MX` falls back to `es`, etc. You only have to translate the regional differences from the base locale.
...
2. **Copy `en.json` to `<your-locale>.json`** following the locale code convention (`fr.json`, `de.json`, `pt_BR.json`, etc.).
...
- **Free admin screens** — `wcpos/woocommerce-pos` repository, at `packages/settings/src/translations/locales/<locale>/wp-admin-settings.json` (and similar paths for other admin packages).
- **Pro admin screens** — `wcpos/woocommerce-pos-pro` repository, with the same pattern (e.g. `packages/store-edit/src/translations/locales/<locale>/wp-admin-store-edit.json`).
