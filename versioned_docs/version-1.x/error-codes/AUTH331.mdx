...
description: "The WCPOS plugin on this store is older than the app and cannot connect."
---
...
The WCPOS plugin on this store is older than the app and cannot connect.

This app speaks a newer store API than the WCPOS plugin installed on the site provides. Ask the site administrator to update the WCPOS plugin in WP Admin, then connect again. Nothing needs to change on the till.
...
3. After updating, reconnect from the till — the store address does not need to be re-entered.
4. If the store cannot be updated yet, contact support: the till needs an app build that matches the plugin, and support can advise which one.
