...
Your store refused a request — usually a field on a record it will not accept, sometimes a settings or authentication request.
...
3. If it is not a record request, follow the server's message for the named endpoint instead. The browser's developer tools (Network tab) show the response when the details are not in the POS log.
