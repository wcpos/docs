...
Set it up from **WP Admin → POS → Settings → Stores**, then click into the store you want to configure. Each store has its own:

- **Template assignments** — pick which templates from *Your Templates* are active for this store, with the same drag-handle ordering you have on the main Templates page
- **Letterhead** — store name, address, logo, phone, email used by the bundled templates' header block

When a cashier signs in at a store, only that store's active templates appear in the receipt dropdown. If a store has no per-store assignments, it falls back to the site-wide Templates page.
