...
- **Receipts** can show the recorded regular price and the saving from a line-item price change. This saving is separate from the WooCommerce **Discount** field.
...
### Showing the original price and saving on receipts {#showing-the-original-price-and-saving-on-receipts}

The current bundled price-bearing templates show the recorded regular price and saving when a product is on sale or its price is changed at the till. Coupon discounts remain separate, so a receipt can show both reductions without counting either one twice.
...
If you need discount figures in WooCommerce reports or an order-level discount total, continue to use [Coupons](/coupons). POS savings are deliberately kept separate from WooCommerce's native coupon discount totals.
