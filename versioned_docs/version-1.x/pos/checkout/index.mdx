...
sequenceDiagram
    ...
    participant WC as WooCommerce Server

    User->>POS: Click Checkout
    POS->>WC: Create / update order
    WC-->>POS: Order ID
    POS->>Modal: Open iframe with Order Pay URL
    Modal->>WC: Load Order Pay page
    WC-->>Modal: Payment form & gateways
    User->>Modal: Select payment method & pay
    Modal->>WC: Process payment
    WC-->>Modal: Payment result
    Modal-->>POS: Order completed
    POS->>User: Show receipt
```
