...
| `selling_price` / `_incl` / `_excl` | number/null | Recorded selling unit price before coupons |
...
Every money field has a formatted `_display` companion, including `regular_price_display`, `selling_price_display`, `unit_savings_display`, `line_regular_total_display`, `line_selling_total_display`, and `line_savings_display`. Inclusive and exclusive fields also have `_display` variants.
...
The bundled price-bearing templates use these guards and display **Total saved** only when the complete value is greater than zero.
