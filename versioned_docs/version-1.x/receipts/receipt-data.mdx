...
Receipt Data v1.2 adds `discount_type` to each discount row: the WooCommerce coupon type behind the discount.
...
## Extending receipt data {#extending}

WCPOS keeps the contract above vendor-neutral. When a plugin needs the receipt to know something WooCommerce does not model, it adds that itself through the `woocommerce_pos_receipt_data` filter, the same way plugins filter WooCommerce's own order totals. The filter runs on every receipt the server builds: live receipts, fiscal snapshots at payment time, PDF downloads, and legacy PHP templates.

```php
add_filter( 'woocommerce_pos_receipt_data', function ( array $data, WC_Abstract_Order $order, string $mode ) {
    foreach ( $data['discounts'] as &$row ) {
        if ( 'smart_coupon' === $row['discount_type'] ) {
            $row['gift_card'] = true;
        }
    }
    return $data;
...
A template can then guard on the new key like any other section:
...
Keys defined in the schema should keep their documented types. Added keys are not part of the schema, so they do not appear in the template editor's field picker, and fiscal snapshots only carry what the filter added at the time the order was paid. The filter is available from WCPOS 1.10.8.
...
<PriceFlow
  ...
  summary={[
    { label: "Sale savings across the order", value: '`totals.sale_savings_total` = 5.00' },
...
| `discount_type` | string | WooCommerce coupon type slug, for example `fixed_cart`, `percent`, or `fixed_product`. Empty when the coupon no longer exists. |
...
`discount_type` was added in Receipt Data v1.2. Fiscal receipt snapshots captured before then do not carry it.
...
The pre-formatted money fields (the `*_display` values such as `totals.total_display`) render amounts with the **currency symbol** in the order's currency, using the position and separators above — so receipts show `$12.00`, not `USD 12.00`. The raw `order.currency` field remains the ISO code for any logic in your template.
