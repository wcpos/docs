...
| `regular_price` / `_incl` / `_excl` | number/null | Recorded regular unit price |
| `selling_price` / `_incl` / `_excl` | number/null | Pre-coupon selling unit price — the recorded POS price when available, otherwise derived from the stored line subtotal |
| `unit_savings` / `_incl` / `_excl` | number/null | `regular_price - selling_price`, clamped to zero |
| `line_regular_total` / `_incl` / `_excl` | number/null | Recorded regular price multiplied by quantity |
| `line_selling_total` / `_incl` / `_excl` | number/null | Pre-coupon selling price multiplied by quantity |
| `line_savings` / `_incl` / `_excl` | number/null | Total regular-to-selling-price savings for this line |
| `savings_in_discounts` | boolean | True when a legacy order already includes the same saving in `discounts` |
| `unit_subtotal` / `_incl` / `_excl` | number | WooCommerce pre-discount unit value; do not use this as the regular price |
| `unit_price` / `_incl` / `_excl` | number | Final unit value after WooCommerce discounts |
| `line_subtotal` / `_incl` / `_excl` | number | WooCommerce pre-discount line subtotal |
| `discounts` / `_incl` / `_excl` | number | Native WooCommerce line discount as a positive value; normally coupon-only on current orders |
...
Every money field has a formatted `_display` companion, including `regular_price_display`, `selling_price_display`, `unit_savings_display`, `line_regular_total_display`, `line_selling_total_display`, and `line_savings_display`. Inclusive and exclusive fields also have `_display` variants. When a nullable savings field is `null`, its `_display` companion is omitted entirely.

Savings fields are `null` when the order does not contain enough recorded POS price data. Templates should therefore wrap them in Mustache sections rather than assuming that every historical order has a regular price.
...
| `totals.savings_total` / `_incl` / `_excl` | number/null | Total recorded regular-to-selling-price savings across product lines, including catalogue sales and till price changes |
| `totals.total_saved` / `_incl` / `_excl` | number/null | Combined WooCommerce discounts and regular-price savings, with legacy overlap removed |
| `totals.total_saved_complete` | boolean | Whether the receipt has enough recorded price data to calculate the complete savings totals |
...
Formatted variants include `subtotal_display`, `discount_total_display`, `savings_total_display`, `total_saved_display`, `tax_total_display`, `total_display`, `paid_total_display`, `change_total_display`, `refund_total_display`, and `net_total_display`, plus inclusive/exclusive variants where applicable.
