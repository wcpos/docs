...
sidebar_position: 2
description: Print WCPOS receipts to printers that aren't attached to the till — Star CloudPRNT, Epson Server Direct Print, and PrintNode.
---
...
- **PrintNode.** WCPOS submits the print job to the [PrintNode](https://www.printnode.com/) cloud service, and a small desktop client running on a computer near the printer picks it up and prints it. The printer can be almost anything that computer can already print to.
...
Star and Epson printers can only use **thermal** templates, because the job has to be rendered to the printer's native command language (ESC/POS for Star, ePOS-Print for Epson). PrintNode can use **any** template — thermal or full-page HTML — because the job is rendered to a **PDF**. If a template doesn't appear as an option for a printer, it's because the printer can't render that format. See [Thermal Templates](/receipts/thermal-templates) for creating thermal layouts.
:::
