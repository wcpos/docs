...
Start at **[Customise Your Receipt](/receipts/customise)**. It walks through the three easiest ways to do it — picking a different bundled template, asking AI to tweak one, or editing by hand.
:::
...
<FeatureGrid>
  <FeatureCard icon="receipt" title="Standard Receipt">
    **HTML** · Default receipt — logo, store identity, items, totals, payment.
  </FeatureCard>
  <FeatureCard icon="receipt" title="Standard Receipt (RTL)">
...
4. **Print or display** — receipts open after checkout with options to print, email, or view on screen. Your templates also work beyond the till: online customers can [download receipts from My Account](/receipts/storefront) rendered with the same design.

Templates render against a standardised [receipt data payload](/receipts/receipt-data) with sections for store info, line items, totals, tax, payments, refunds, and more. Currency fields all include pre-formatted `_display` variants (e.g., `$29.99` instead of `29.99`).
