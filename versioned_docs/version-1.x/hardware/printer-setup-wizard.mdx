...
<PrinterWizard>
  ...
  <WizardQuestion id="where" title="Where is the printer?">
    <WizardChoice value="here" label="Right here — plugged into or on the same wifi/network as this till" goTo="what-print" />
    <WizardChoice value="elsewhere" label="Somewhere else — on another wifi/network, or connected to another computer" goTo="cloud-provider" />
  </WizardQuestion>

  <WizardQuestion id="what-print" title="What do you want to print?">
    <WizardChoice value="receipts" label="Receipts for the sales I ring up at this till" goTo="platform" />
    <WizardChoice value="auto" label="Print orders automatically — online-store orders, or to a kitchen or back-office printer" goTo="cloud-provider" />
  </WizardQuestion>
...
Then confirm the printer is reachable: open `http://<printer-ip>/` in a web browser (if that doesn't load, try `https://<printer-ip>/`). If you see the printer's own configuration page, it's on the network and the problem is in the settings; if neither loads, it's a network/IP problem (wrong address, or the printer isn't connected).
...
**The printer's network settings use the wrong protocol.** WCPOS sends jobs over **raw TCP on port 9100**. Open the printer's configuration page in a browser (`http://<printer-ip>/`, or `https://<printer-ip>/` if that doesn't load) and make sure **raw / port 9100** printing is enabled — if the printer is set only to LPR, IPP, or WSD, the job never arrives.
...
First, confirm the printer is reachable: open `http://<printer-ip>/` in a new browser tab (if that doesn't load, try `https://<printer-ip>/`). If you see the printer's own configuration page, it's on the network; if neither loads, fix the network/IP first.
...
The web app can also only reach a printer through its **built-in web server** — Epson's **ePOS-Print** or Star's **WebPRNT** — so make sure that service is switched **on** in the printer's configuration. Only Epson "intelligent" / **-NT** models and Star **WebPRNT** models have one at all; if yours doesn't, a browser can't reach it.
...
First, **pair the printer in your computer's or device's system Bluetooth settings** — not in WCPOS. Bluetooth printers only remember one device at a time, so if it was paired to another till, "forget" it there first. On **iPhone/iPad**, also allow WCPOS both **Bluetooth** and **Local Network** permission when prompted. On **Android**, set WCPOS to **unrestricted** battery use so the system doesn't drop the connection. On a **Windows or Mac desktop**, pair it in the operating system's Bluetooth settings first, then scan in WCPOS.
...
Plug the printer in by USB. On **Android**, approve the USB permission prompt when it appears, then pick the printer in **POS &gt; Settings &gt; Printing**. On a **Mac**, USB usually works with no extra setup. On **Windows**, a generic printer may need its driver installed **before** you plug it in.
...
Connecting a printer to the **web app** over Bluetooth or USB works only in **Google Chrome or Microsoft Edge** (Safari and Firefox can't do it). Click **Connect** in **POS &gt; Settings &gt; Printing** and choose the printer in the browser's device chooser.

**USB** works with Epson, Star, and most generic ESC/POS printers — if your printer appears in the browser's chooser, it should print. On **Windows**, the printer's own driver can block the browser from talking to it; if connecting fails there, the desktop app is the way to go.

**Bluetooth** depends on the printer's Bluetooth type, not the brand: the browser can only reach **Bluetooth Low Energy (BLE)** printers. Many small portable printers are BLE and work fine, but printers with classic Bluetooth — including most Bluetooth Epson and Star models — never appear in the browser's chooser. If yours doesn't show up, that's why: use the desktop or mobile app for it instead.

If it won't connect either way, the **desktop app** is the most reliable option for Bluetooth and USB. Download it from [Printer Setup](/hardware/printers).
...
Your printer is connected and printing correctly.

**Want receipts to print automatically?** Turn on **Auto-print receipt** in the POS cart settings, so a receipt prints the moment you complete a sale.

If it's a **network** printer, give it a **DHCP reservation** on your router so its address never changes and breaks printing later — Bluetooth and USB printers don't need this. And if you use more than one template, see [print routing](/hardware/printers#print-routing).
...
Your cloud printer is connected and printing. It appears automatically in every device's printer list — you don't add it per device.

Set up an [auto-print rule](/receipts/cloud-printing#auto-print) so orders print automatically. With [WCPOS Pro](/getting-started/pro-license), each store can have its own rules and choose exactly what prints — **POS orders only**, **online orders only**, or **every order** — see [per-store printers](/receipts/cloud-printing#per-store-printers).
