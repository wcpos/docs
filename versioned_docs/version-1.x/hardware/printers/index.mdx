...
:::tip New to this? Try the guided Setup Wizard
Not sure where to start? The [**Printer Setup Wizard**](/hardware/printers/setup-wizard) asks a few questions and walks you through connecting your printer step by step — and helps you fix it if the test print doesn't work.
:::
...
|----------|--------------------|-------------|
| **Web app** | Epson and Star printers with a built-in web server (network); most ESC/POS printers via WebUSB (USB); Bluetooth Low Energy (BLE) printers via Web Bluetooth (USB/Bluetooth are Chrome/Edge only) | Network (HTTP), USB, Bluetooth |
...
USB and Bluetooth printing in the browser work too, but only in **Chrome or Microsoft Edge** (they use the WebUSB and Web Bluetooth APIs) — Safari and Firefox don't support them. Over **USB** this isn't limited to Epson and Star: most ESC/POS printers that appear in the browser's device chooser will print. Over **Bluetooth**, what matters is the printer's Bluetooth type — the browser can only reach **Bluetooth Low Energy (BLE)** printers; classic-Bluetooth models (including most Bluetooth Epson and Star printers) can't connect from a browser.
...
- **Newer models (TM-m30III and similar):** these serve ePOS on port `80` (HTTP) / `443` (HTTPS) instead of 8008/8043 — if the auto-filled port doesn't connect on the web app, set the **Port** to `80` (HTTP store) or `443` (HTTPS store). For HTTPS, first open `https://<printer-ip>` on each POS device and accept the printer's certificate warning. If the browser cannot trust the printer, use the **desktop app** instead.

**Confirmed working models:**
- Epson TM-T70-i 2 (network)
- Epson TM-m30III (network — direct, no middleware required)
...
:::warning New Epson printer prints nothing?
Epson printers manufactured since mid-2025 ship with **Secure Printing** enabled, which silently discards network print jobs — test prints "send" but nothing comes out. See [Epson Secure Printing](/hardware/printers/secure-printing) for the two-minute fix.
:::
...
<Accordion>
  ...
  - For the web app over the **network**: confirm the printer is an Epson or Star model with ePOS/WebPRNT support (USB and Bluetooth on the web are not brand-limited — see below)
  ...
  - **New Epson printer?** Recent Epson models ship with **Secure Printing** enabled, which silently discards network print jobs while the printer reports itself healthy — see [Epson Secure Printing](/hardware/printers/secure-printing) for the fix
  ...
  <AccordionItem question="New Epson printer: test print says sent, nothing comes out">

  This is the signature of **Secure Printing**, enabled from the factory on Epson printers manufactured since mid-2025 (an EU security regulation). The printer silently holds and discards jobs sent over the classic channels — no error anywhere, and its status page says "Available". Panel self-tests still print. The fix takes two minutes in the printer's web settings: [Epson Secure Printing](/hardware/printers/secure-printing).
  ...
  - Over **Bluetooth**, the browser can only reach **Bluetooth Low Energy (BLE)** printers — classic-Bluetooth models (including most Bluetooth Epson and Star printers) never appear in the browser's chooser; use the desktop or mobile app for those
