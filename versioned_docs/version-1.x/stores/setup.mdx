...
- **The cashiers who'll work at this store have WordPress accounts** with a role that includes the *Access WCPOS* capability. If you haven't set that up yet, see [Access Settings](/settings/wp-admin/access).
...
- **Authorized Users** — checkbox list of every WordPress user whose role has the *Access WCPOS* capability. Check the cashiers who should be able to sign in at this store. **Leaving the list empty means all POS-capable users are authorized** — handy for single-store sites.
