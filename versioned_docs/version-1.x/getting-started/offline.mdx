...
- **Applying coupons** (Pro) — codes validate locally against synced coupon data; the server re-validates when the order is submitted
...
- **Completing checkout** — processing payment and creating the order in WooCommerce requires server communication (applied coupons are re-validated here)
