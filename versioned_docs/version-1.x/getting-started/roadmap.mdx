...
- **Offline checkout** — complete a sale without a connection, starting with cash, and let it sync when you're back online
...
<LinkCards>
  <LinkCard to="/reference/sync-engine" title="New sync engine (v1.10)" description="Rebuilt syncing: lighter on your server, honest download coverage, offline write recovery." icon="sync" />
  <LinkCard to="/support/store-health" title="Store health (v1.10)" description="Sync presets, measured server load, coverage, and the activity log." icon="heart-pulse" />
  <LinkCard to="/settings/wp-admin/checkout#prevent-overselling" title="Prevent overselling (v1.10)" description="Block out-of-stock items at the cart, with the server validating stock at checkout." icon="box" />
  ...
  <LinkCard to="/receipts/thermal-templates" title="Thermal receipts" description="Design thermal receipt layouts for Epson and Star printers." icon="receipt" />
</LinkCards>