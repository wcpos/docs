...
3. Sign in to WCPOS at the right store (multi-store sites) — see [Multi-Store Setup](/stores/setup) if you're not sure which one to pick.
...
- If your gateway requires manual batch settlement (older Stripe/Square terminals), **settle the batch now**, before you go home. Integrated gateways like Stripe Terminal auto-settle.

  </StepCard>
  <StepCard number="6" title="Print the report">

In the Reports screen Summary panel, click **Print**. This sends the report through your normal receipt or document printer (per [Print Routing](/templates/printer-setup#print-routing)) and produces a paper record.
...
- [Multi-Store Setup](/stores/setup) — per-store reconciliation
