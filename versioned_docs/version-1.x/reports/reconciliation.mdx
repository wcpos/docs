...
<ComingSoon>Cash float entry and formal shift start/stop are planned — they'll record the opening float in-app and tie each sale to a shift for accurate end-of-day totals.</ComingSoon>

## Before your first close {#before-your-first-close}
...
## Opening the till {#opening-the-till}
...
## During the shift {#during-the-shift}
...
- If you use a manual batch-settling terminal, **settle the batch now**, before you go home. Integrated gateways like Stripe Terminal auto-settle. For Square configuration through Web Checkout, see [Web Checkout](/payment/gateways/web-checkout).

</StepCard>
  <StepCard number="6" title="Print the report">

In the Reports screen Summary panel, click **Print**. This typically uses your configured print routing (see [Print Routing](/hardware/printers#print-routing)) to send the report to a receipt or document printer and produces a paper record.
...
## Variance handling {#variance-handling}
...
5. **Coupons and discounts.** Confirm the totals account for any discounts applied — the report's *Total Discounts* line shows tracked coupon discounts; line-item price overrides reduce the *Net Sales* directly (see [Discounts](/pos/cart/discounts#why-tracked-differently)).
...
## Multi-cashier or multi-store scenarios {#multi-cashier-or-multi-store-scenarios}
