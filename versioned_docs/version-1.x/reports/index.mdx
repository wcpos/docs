...
#### 1. Sales Chart (Top) {#1-sales-chart-top}
...
#### 2. Orders Table (Bottom Left) {#2-orders-table-bottom-left}
...
#### 3. Report Summary (Bottom Right) {#3-report-summary-bottom-right}
...
:::note Complete reports fetch every order when online
When you're connected and a report's date range covers more orders than are on the device, WCPOS fetches every matching order to completion — there's no 10,000-record ceiling — showing progress as it works, so the totals reflect the full range rather than a truncated slice. Offline, the default report stays limited to the orders already synced to this device.
:::

## End-of-Day Workflow {#end-of-day-workflow}

For the full cashier-facing walkthrough — opening float, drawer count, cash + card reconciliation, variance handling, and sign-off — see **[End-of-Day Reconciliation](/reports/reconciliation)**.
