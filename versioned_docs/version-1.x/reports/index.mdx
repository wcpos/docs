...
:::note Large reports fetch to completion
When a report's date range covers more orders than are on the device, WCPOS fetches every matching order to completion — there's no 10,000-record ceiling — and shows progress while it works, so the totals reflect the full range rather than a truncated slice.
:::
