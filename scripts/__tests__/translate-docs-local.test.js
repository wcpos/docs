...
function assertFailed(result, reason, withWorktree = true) {
  ...
  const summary = `[translate-docs-local] FAILED: ${reason}` + (withWorktree ? ` (worktree: ${wt}, logs: ${wt}/.translate/logs)` : '');
  for (const output of [result.stdout, result.stderr]) assert.equal(output.trimEnd().split('\n').at(-1), summary);
}
...
if (tool === 'git') {
  ...
  if (args[0] === 'scripts/docs-translation/worklist.js') {
    if (config.worklistFails) process.exit(3);
  ...
  if (config.tamper && prompt.startsWith(config.tamper)) fs.writeFileSync('.translate/plan.json', '{"tampered":true}');
...
it.each(['TRANSLATE', 'REVIEW'])('rejects plan edits made during %s before apply', tamper => {
  const result = run({ counts: [1], tamper });
  assertFailed(result, 'plan.json changed during model calls');
  assert.equal(models(result).length, 2);
  assert.ok(!result.calls.some(c => isCall(c, 'node', 'scripts/docs-translation/apply.js') || isCall(c, 'git', 'commit') || isCall(c, 'git', 'push')));
});

it('reports an unexpected worklist failure on both streams', () => {
  const result = run({ worklistFails: true });
...
it.each([
  ...
  [['--translator', 'unknown'], 'unknown translator: unknown'],
  ...['', '-1', 'abc'].map(value => [['--max-units', value], 'max-units must be a positive integer']),
  [['--max-units', '0'], 'max-units must be positive'],
])('reports argument failure before creating a worktree: %j', (args, reason) => {
  const result = run({}, args);
  assertFailed(result, reason, false);
...
it('aborts a merge conflict and exits before installing or calling a model', () => {
  ...
  assertFailed(result, `merge failed for PR ${pr.number} (${pr.url})`);
...
it('refuses to push to the base and supports skipping review', () => {
  ...
  assertFailed(result, 'refusing to push to ' + pr.headRefName);
  assert.equal(result.status, 1, result.stderr);
  assert.equal(models(result).length, 2);
...
it('skips review without a results file and fails when nothing is accepted', () => {
  ...
  assertFailed(result, 'no translations accepted');
  assert.equal(result.status, 1, result.stderr);
  assert.equal(models(result).length, 2);
